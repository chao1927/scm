...
/**
 * BidComparisonQueryApplicationService。
 *
...
public class BidComparisonQueryApplicationService {
    ...
    /**
     * readModel（类型：{@code BidComparisonReadModelPort}）。
     *
    ...
    /**
     * 创建 BidComparisonQueryApplicationService。
     *
     * <p>构造阶段集中接收必需依赖或恢复对象状态，确保实例创建后即可安全参与所属用例。
     * @param readModel 业务处理参数或成员，类型为 {@code BidComparisonReadModelPort}
     */
    ...
    public PageResult<BidComparisonView> page(Long purchaseOrgId, Long scope, Integer status, String rfqNo, int pageNo, int pageSize) {
    ...
    public BidComparisonView detail(String compareNo, Long scope) {
        var view = readModel.detail(compareNo).orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND, "比价结果不存在"));
