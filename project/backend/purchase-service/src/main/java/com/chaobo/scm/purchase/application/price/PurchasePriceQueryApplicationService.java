...
/**
 * PurchasePriceQueryApplicationService。
 *
...
public class PurchasePriceQueryApplicationService {
    ...
    /**
     * readModel（类型：{@code PurchasePriceReadModelPort}）。
     *
    ...
    /**
     * 创建 PurchasePriceQueryApplicationService。
     *
     * <p>构造阶段集中接收必需依赖或恢复对象状态，确保实例创建后即可安全参与所属用例。
     * @param readModel 业务处理参数或成员，类型为 {@code PurchasePriceReadModelPort}
     */
    ...
    public PageResult<PurchasePriceView> page(Long purchaseOrgId, Long scope, Long supplierId, String skuCode, String currency, Integer status, int pageNo, int pageSize) {
    ...
    public PurchasePriceView detail(String priceNo, Long scope) {
        var view = readModel.detail(priceNo).orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND, "采购价格不存在"));
