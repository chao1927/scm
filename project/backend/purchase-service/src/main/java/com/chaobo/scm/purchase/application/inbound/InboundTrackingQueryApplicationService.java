...
/**
 * InboundTrackingQueryApplicationService。
 *
...
public class InboundTrackingQueryApplicationService {
    ...
    /**
     * readModel（类型：{@code InboundTrackingReadModelPort}）。
     *
    ...
    /**
     * 创建 InboundTrackingQueryApplicationService。
     *
     * <p>构造阶段集中接收必需依赖或恢复对象状态，确保实例创建后即可安全参与所属用例。
     * @param readModel 业务处理参数或成员，类型为 {@code InboundTrackingReadModelPort}
     */
    ...
    public PageResult<InboundTrackingView> page(Long purchaseOrgId, Long scope, String orderNo, String asnNo, String warehouseCode, Integer status, int pageNo, int pageSize) {
    ...
    public InboundTrackingView detail(String inboundNo, Long scope) {
        var view = readModel.detail(inboundNo).orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND, "到货跟踪不存在"));
