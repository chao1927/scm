...
/**
 * PurchaseRequisitionQueryMapper。
 *
...
public interface PurchaseRequisitionQueryMapper {
    ...
    @Select("""
        ...
        """)
    long count(@Param("purchaseOrgId") Long purchaseOrgId, @Param("status") Integer status, @Param("keyword") String keyword);
     ...
     * @return 处理当前类型职责中的操作的结果，类型为 {@code List<PurchaseRequisitionMapper.HeaderRow>}
     */
    @Select("""
        <script>
        select *
        ...
        """)
    List<PurchaseRequisitionMapper.HeaderRow> page(@Param("purchaseOrgId") Long purchaseOrgId, @Param("status") Integer status, @Param("keyword") String keyword, @Param("offset") int offset, @Param("limit") int limit);
}
