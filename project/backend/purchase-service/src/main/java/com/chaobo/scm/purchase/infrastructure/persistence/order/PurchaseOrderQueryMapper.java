...
public interface PurchaseOrderQueryMapper {
    ...
    @Select("""
        ...
        """)
    long count(@Param("purchaseOrgId") Long purchaseOrgId, @Param("supplierId") Long supplierId, @Param("status") Integer status);
     ...
     * @return 处理当前类型职责中的操作的结果，类型为 {@code List<PurchaseOrderMapper.HeaderRow>}
     */
    @Select("""
        <script>
        ...
        """)
    List<PurchaseOrderMapper.HeaderRow> page(@Param("purchaseOrgId") Long purchaseOrgId, @Param("supplierId") Long supplierId, @Param("status") Integer status, @Param("offset") int offset, @Param("limit") int limit);
}
