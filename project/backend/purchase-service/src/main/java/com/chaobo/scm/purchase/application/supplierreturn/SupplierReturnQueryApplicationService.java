...
/**
 * SupplierReturnQueryApplicationService。
 *
...
public class SupplierReturnQueryApplicationService {
    ...
    /**
     * readModel（类型：{@code SupplierReturnReadModelPort}）。
     *
    ...
    /**
     * 创建 SupplierReturnQueryApplicationService。
     *
     * <p>构造阶段集中接收必需依赖或恢复对象状态，确保实例创建后即可安全参与所属用例。
     * @param readModel 业务处理参数或成员，类型为 {@code SupplierReturnReadModelPort}
     */
    ...
    public PageResult<SupplierReturnView> page(Long purchaseOrgId, Long scope, Long supplierId, String warehouseCode, Integer status, int pageNo, int pageSize) {
    ...
    public SupplierReturnView detail(String returnNo, Long scope) {
        var view = readModel.detail(returnNo).orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND, "退供申请不存在"));
