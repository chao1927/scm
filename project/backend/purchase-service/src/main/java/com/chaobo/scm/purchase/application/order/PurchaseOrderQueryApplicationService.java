...
/**
 * PurchaseOrderQueryApplicationService。
 *
...
public class PurchaseOrderQueryApplicationService {
    ...
    /**
     * readModel（类型：{@code PurchaseOrderReadModelPort}）。
     *
    ...
    /**
     * 创建 PurchaseOrderQueryApplicationService。
     *
     * <p>构造阶段集中接收必需依赖或恢复对象状态，确保实例创建后即可安全参与所属用例。
     * @param readModel 业务处理参数或成员，类型为 {@code PurchaseOrderReadModelPort}
     */
    ...
    public PageResult<PurchaseOrderView> page(Long purchaseOrgId, Long scope, Long supplierId, Integer status, int pageNo, int pageSize) {
    ...
    public PurchaseOrderView detail(String orderNo, Long scope) {
        var view = readModel.detail(orderNo).orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND, "采购订单不存在"));
