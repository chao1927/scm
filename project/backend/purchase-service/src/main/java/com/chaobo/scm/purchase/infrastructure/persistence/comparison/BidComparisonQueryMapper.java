...
public interface BidComparisonQueryMapper {
     ...
     * @return 处理当前类型职责中的操作的结果，类型为 {@code List<BidComparisonMapper.HeaderRow>}
     */
    @Select("""
        <script>
        select *
        ...
        """)
    List<BidComparisonMapper.HeaderRow> page(@Param("purchaseOrgId") Long purchaseOrgId, @Param("status") Integer status, @Param("rfqNo") String rfqNo, @Param("offset") int offset, @Param("limit") int limit);
}
