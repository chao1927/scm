...
public class ReverseAfterSaleController {
     ...
     */
    @PostMapping("/internal/oms/v1/reverse-after-sale-events")
    public void consume(@RequestBody Event body, Authentication authentication) {
        String source = switch(body.eventType()) {
        ...
        ScmAccessContexts.require(authentication).requireApplication(source);
        service.consume(new ReverseAfterSaleApplicationService.Event(body.eventId(), body.eventType(), body.afterSaleNo(), body.receivedQty(), body.acceptedQty(), body.amount(), body.unmatched(), body.payload()));
    }
