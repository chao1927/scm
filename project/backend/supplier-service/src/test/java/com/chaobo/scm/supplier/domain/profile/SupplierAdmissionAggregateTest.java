...
/**
 * SupplierAdmissionAggregateTest。
 *
...
class SupplierAdmissionAggregateTest {
    ...
    /**
     * 处理当前类型职责中的操作 {@code admission}。
     *
     * <p>该内部步骤用于收敛重复逻辑或保护局部规则，调用方应通过当前类型公开的业务入口使用该能力。
     * @return 处理当前类型职责中的操作的结果，类型为 {@code SupplierAdmissionAggregate}
     */
    private SupplierAdmissionAggregate admission() {
        return SupplierAdmissionAggregate.create("SUP-C1", "候选供应商", "91310000", "MANUFACTURER", "张三", "13800000000", "{\"taxNo\":\"91310000\"}", 1, ids);
    }

    /**
     * 处理当前类型职责中的操作 {@code shouldSubmitAndApprove}。
     *
    ...
    /**
     * 处理当前类型职责中的操作 {@code shouldNotApprovePotentialSupplier}。
     *
     * <p>该方法完成当前用例中的一个明确业务动作；状态修改、权限、幂等和异常语义由所属层次共同约束。
     */
    @Test
