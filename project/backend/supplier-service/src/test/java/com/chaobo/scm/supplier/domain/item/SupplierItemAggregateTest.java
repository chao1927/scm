...
class SupplierItemAggregateTest {
    ...
    private SupplyCondition condition() {
        return new SupplyCondition(new BigDecimal("10"), new BigDecimal("2"), 7, "件", null, null);
    }

    /**
     * 处理当前类型职责中的操作 {@code shouldPauseAndResume}。
     *
     * <p>该方法完成当前用例中的一个明确业务动作；状态修改、权限、幂等和异常语义由所属层次共同约束。
     */
    @Test
    void shouldPauseAndResume() {
        var a = SupplierItemAggregate.enable(10, "SKU-1", "SP-1", condition(), 1, ids);
        a.pause("产能不足", 1, ids);
        assertThat(a.status()).isEqualTo(SupplyStatus.PAUSED);
        a.resume(1, ids);
        assertThat(a.status()).isEqualTo(SupplyStatus.AVAILABLE);
    }

    /**
     * 处理当前类型职责中的操作 {@code shouldNotPauseTwice}。
     *
     * <p>该方法完成当前用例中的一个明确业务动作；状态修改、权限、幂等和异常语义由所属层次共同约束。
     */
    @Test
    void shouldNotPauseTwice() {
        var a = SupplierItemAggregate.enable(10, "SKU-1", "SP-1", condition(), 1, ids);
        a.pause("产能不足", 1, ids);
        assertThatThrownBy(() -> a.pause("重复", 1, ids)).isInstanceOf(BusinessException.class);
    }
