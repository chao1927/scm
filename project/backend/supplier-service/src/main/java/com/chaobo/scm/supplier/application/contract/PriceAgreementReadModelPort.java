 ...
 * @since 0.1.0
 */
public interface PriceAgreementReadModelPort {

    /**
     * 处理当前类型职责中的操作 {@code detail}。
     *
     * <p>该方法完成当前用例中的一个明确业务动作；状态修改、权限、幂等和异常语义由所属层次共同约束。
     * @param agreementId 业务或技术标识，类型为 {@code long}
     * @return 处理当前类型职责中的操作的结果，类型为 {@code Optional<PriceAgreementView>}
     */
    Optional<PriceAgreementView> detail(long agreementId);
     ...
     */
    PageResult<PriceAgreementView> page(Long supplierId, String skuCode, int pageNo, int pageSize);
     ...
     */
    Optional<PriceAgreementView.Line> activeLine(long supplierId, String skuCode, String currency, LocalDate businessDate);
}
