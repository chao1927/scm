...
public enum QualityIssueStatus {
    ...
    // 业务枚举值：rectifying
    RECTIFYING(2, "整改中"),
    // 业务枚举值：pending verification
    PENDING_VERIFICATION(3, "待审核"),
    // 业务枚举值：closed
    CLOSED(4, "已关闭"),
    // 业务枚举值：overdue
    OVERDUE(5, "已逾期");
     ...
     * <p>该方法完成当前用例中的一个明确业务动作；状态修改、权限、幂等和异常语义由所属层次共同约束。
     * @param code 可追踪业务编码，类型为 {@code int}
     * @return 转换数据模型的结果，类型为 {@code QualityIssueStatus}
     */
    public static QualityIssueStatus fromCode(int code) {
        for (var v : values()) {
            if (v.code == code) {
                return v;
            }
        }
        throw new IllegalArgumentException("未知质量问题状态");
    }
}
