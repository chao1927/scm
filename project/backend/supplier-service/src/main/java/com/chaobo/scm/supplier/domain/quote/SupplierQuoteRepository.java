...
public interface SupplierQuoteRepository {
     ...
     */
    Optional<SupplierQuoteAggregate> findById(long id);

    /**
     * 处理当前类型职责中的操作 {@code expiredIds}。
     *
     * <p>该方法完成当前用例中的一个明确业务动作；状态修改、权限、幂等和异常语义由所属层次共同约束。
     * @return 处理当前类型职责中的操作的结果，类型为 {@code java.util.List<Long>}
     */
    java.util.List<Long> expiredIds();
     ...
     */
    void save(SupplierQuoteAggregate aggregate, long operatorId);
}
