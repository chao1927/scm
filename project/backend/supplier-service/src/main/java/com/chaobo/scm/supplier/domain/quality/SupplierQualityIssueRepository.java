...
/**
 * SupplierQualityIssueRepository。
 *
 ...
 * @since 0.1.0
 */
public interface SupplierQualityIssueRepository {
     ...
     */
    Optional<SupplierQualityIssueAggregate> findById(long id);

    /**
     * 执行命令 {@code save}。
     *
     * <p>该方法完成当前用例中的一个明确业务动作；状态修改、权限、幂等和异常语义由所属层次共同约束。
     * @param aggregate 业务处理参数或成员，类型为 {@code SupplierQualityIssueAggregate}
     * @param operatorId 业务或技术标识，类型为 {@code long}
     */
    void save(SupplierQualityIssueAggregate aggregate, long operatorId);
}
