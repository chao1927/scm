...
/**
 * QualitySourceEventConsumerApplicationService。
 *
...
public class QualitySourceEventConsumerApplicationService {
    ...
    /**
     * 创建 QualitySourceEventConsumerApplicationService。
     *
     ...
     */
    public QualitySourceEventConsumerApplicationService(MasterDataEventConsumeLogPort inbox, SupplierQualityIssueApplicationService issues, InboundEventPayloadStore payloads) {
        this.inbox = inbox;
        this.issues = issues;
     ...
     * @param event 业务处理参数或成员，类型为 {@code QualitySourceEvent}
     */
    @Transactional(rollbackFor = Exception.class)
    public void consume(QualitySourceEvent event) {
        if (!Set.of(WMS, OMS).contains(event.sourceSystem())) {
            throw new BusinessException(ErrorCode.BUSINESS_RULE_FAILED, "质量事件来源必须为WMS或OMS");
        }
        ...
        try {
            var context = new CommandContext(0, event.sourceSystem(), 0, null, event.sourceSystem() + "-" + event.eventCode(), null, event.sourceSystem() + ":" + event.eventCode(), Set.of("supplier:quality:create"));
            issues.create(event.supplierId(), event.sourceSystem(), event.sourceNo(), event.issueType(), event.severity(), event.description(), context);
