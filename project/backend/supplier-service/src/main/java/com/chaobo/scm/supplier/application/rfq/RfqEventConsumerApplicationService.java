...
/**
 * RfqEventConsumerApplicationService。
 *
...
public class RfqEventConsumerApplicationService {
    ...
    /**
     * 创建 RfqEventConsumerApplicationService。
     *
     ...
     */
    public RfqEventConsumerApplicationService(MasterDataEventConsumeLogPort inbox, SupplierQuoteTodoMapper todos, IdentifierGenerator ids, ObjectMapper json, InboundEventPayloadStore payloads) {
        this.inbox = inbox;
        this.todos = todos;
        this.ids = ids;
        this.json = json;
     ...
     * @param event 业务处理参数或成员，类型为 {@code RfqEvent}
     */
    @Transactional(rollbackFor = Exception.class)
    public void consume(RfqEvent event) {
        if (!PURCHASE.equals(event.sourceSystem())) {
            throw new BusinessException(ErrorCode.BUSINESS_RULE_FAILED, "询价事件来源必须为PURCHASE");
        }
        ...
        try {
            if (RFQ_PUBLISHED.equals(event.eventType())) {
                todos.upsert(ids.nextId(), event.rfqId(), event.rfqNo(), event.supplierId(), event.quoteDeadline(), json.writeValueAsString(event.payload()));
            } else if (RFQ_BIDDING_CLOSED.equals(event.eventType())) {
    ...
    /**
     * 业务常量 {@code RFQ_BIDDING_CLOSED}。
     *
     * <p>集中表达当前用例使用的固定业务值，避免含义不明的字面量散落在判断或调用参数中。
     */
    private static final String RFQ_BIDDING_CLOSED = "RfqBiddingClosed";

    /**
     * 业务常量 {@code RFQ_PUBLISHED}。
     *
     * <p>集中表达当前用例使用的固定业务值，避免含义不明的字面量散落在判断或调用参数中。
     */
    private static final String RFQ_PUBLISHED = "RfqPublished";
