 ...
 * @since 0.1.0
 */
public enum SupplierAdmissionStatus {
     ...
     * <p>该方法完成当前用例中的一个明确业务动作；状态修改、权限、幂等和异常语义由所属层次共同约束。
     * @param code 可追踪业务编码，类型为 {@code int}
     * @return 转换数据模型的结果，类型为 {@code SupplierAdmissionStatus}
     */
    public static SupplierAdmissionStatus fromCode(int code) {
        for (var v : values()) {
            if (v.code == code) {
                return v;
            }
        }
        throw new IllegalArgumentException("未知准入状态");
    }
}
