...
public class OutboxDispatchApplicationService {
    ...
    public List<OutboxMessage> claim(int batch, int retries) {
        return store.claim(batch, retries);
    }
    ...
    public void dispatch(OutboxMessage message) {
        try (ScmLogContext ignored = ScmLogContext.openSystem(Long.toString(message.eventId()))) {
            ...
            LOG.info("event=rocketmq_publish operation=supplier_outbox_publish result=SUCCESS eventId={}",
                    message.eventId());
        } catch (Exception e) {
            store.markFailed(message.eventId(), abbreviate(e.getMessage()));
            try (ScmLogContext ignored = ScmLogContext.openSystem(Long.toString(message.eventId()))) {
                LOG.error("event=rocketmq_publish operation=supplier_outbox_publish result=FAILURE eventId={}",
                        message.eventId(), e);
            }
        }
    }
