...
public enum AsnStatus {
     ...
     * <p>该方法完成当前用例中的一个明确业务动作；状态修改、权限、幂等和异常语义由所属层次共同约束。
     * @param code 可追踪业务编码，类型为 {@code int}
     * @return 转换数据模型的结果，类型为 {@code AsnStatus}
     */
    public static AsnStatus fromCode(int code) {
        return Arrays.stream(values()).filter(value -> value.code == code).findFirst().orElseThrow(() -> new BusinessException(ErrorCode.SYSTEM_ERROR, "未知 ASN 状态: " + code));
    }
}
