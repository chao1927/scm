...
| [`project/backend/bin/dev.sh`](../project/backend/bin/dev.sh) | 构建并启动九个 Java 服务 | 推荐 | 使用 JDK 17、本地 Maven 仓库和环境白名单；`SCM_ENV=test|prod` 切换环境 |
...
```bash
cd project/backend
SCM_ENV=test ./bin/dev.sh up
./bin/dev.sh status
```

生产配置本地验证时改为 `SCM_ENV=prod ./bin/dev.sh restart`。脚本执行顺序：校验 `test|prod` → 定位 JDK 17 → 使用本地 Maven 仓库构建对应 profile JAR → 以最小环境白名单启动九服务 → 等待端口监听 → 输出失败日志。业务进程不会继承 MySQL root、Nacos 库账号或完整 `.env`。
...
| RocketMQ | `SCM_ROCKETMQ_ENABLED`、`SCM_ROCKETMQ_ENDPOINTS`、各 Topic/Group |
