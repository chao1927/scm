...
5. RocketMQ 业务链路必须是 Outbox → Broker → Inbox；HTTP 事件入口不能代替生产消息链路。
...
2. 应用账号只拥有自身 schema 的 DML/DDL 迁移所需权限。
3. 首次启动前确认所有 Flyway 迁移编号无冲突。
4. 生产变更先备份、再执行迁移；破坏性字段修改采用扩展—迁移—收缩，不直接回滚已被业务写入的数据结构。
5. 发布失败时优先应用向前修复迁移；只有迁移明确可逆且未产生新业务数据时才允许回退。
...
| MySQL | 登录、schema、Flyway、备份 | 应用账号可用、迁移成功、恢复样本可读 |
...
| 服务启动但业务失败 | Nacos配置、Flyway、下游依赖、权限 | 可选配置掩盖缺失、数据库迁移不完整 |
