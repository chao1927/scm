...
> 本文根据 [BMS领域模型](../03-核心业务模型/07-BMS领域模型/01-BMS领域模型.md)、[07-BMS系统产品功能设计](../04-子系统功能设计/07-BMS系统/01-BMS系统产品功能设计.md)、[07-BMS系统数据库设计](../05-子系统数据库设计/07-BMS系统数据库设计.md)、[07-BMS系统接口设计](../06-子系统接口设计/07-BMS系统接口设计.md) 和 [上下文映射与领域事件目录](../06-子系统接口设计/00-上下文映射与领域事件目录.md) 整理。本文专门说明 BMS 在聚合、领域服务、应用服务执行命令后如何生产事件，消费外部事件后如何改变费用、对账、账单、发票和财务交接数据，事件包含哪些字段属性，以及事件如何落表、发布、重试和审计。
...
4. 若计费对象缺失、规则未发布、账期已关闭，则 `bms_fee.process_status=3`，等待完善主数据、规则或账期后重放。
...
| 方向 | 事件 | 触发或消费逻辑 | 数据变化 |
| --- | --- | --- | --- |
| 消费 | `LogisticsFeeSourceGenerated` / `LogisticsFeeSourcePushed` | TMS 推送物流费用来源 | 写入 `bms_event_consume_log` 和费用采集记录，生成或等待生成费用明细 |
| 消费 | `TransportSigned` | 运单满足签收计费条件 | 费用明细可进入待对账或待确认 |
| 消费 | `TransportRejected` | 拒收、二次配送、退回等费用口径变化 | 根据计费规则生成退回费、二次配送费或免计费 |
| 消费 | `LogisticsExceptionClosed` | TMS 关闭异常并确定责任方 | 生成扣罚、赔付、索赔或费用调整依据 |
| 生产 | `BmsFeeSourceCollected` | BMS 成功采集 TMS 费用来源 | 回写 TMS 标记费用来源已采集、冻结直接修改 |
| 生产 | `BmsReconciliationDifferenceCreated` | 物流商对账出现差异 | 通知 TMS 标记费用来源差异，必要时生成物流异常或修正版本 |
