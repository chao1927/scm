...
> 本文根据 [供应商领域模型](../03-核心业务模型/01-供应商领域模型/01-供应商领域模型.md)、[供应商系统产品功能设计](../04-子系统功能设计/供应商系统/供应商系统产品功能设计.md)、[供应商系统数据库设计](../05-子系统数据库设计/01-供应商系统数据库设计.md)、[供应商系统接口设计](../06-子系统接口设计/56-供应商系统接口设计.md) 和 [上下文映射与领域事件目录](../06-子系统接口设计/50-上下文映射与领域事件目录.md) 整理。本文专门说明供应商系统在聚合、领域服务、应用服务执行命令后如何生产事件，消费外部事件后如何改变本地数据，事件包含哪些字段属性，以及事件如何落表、发布、重试和审计。
...
| --- | --- |
| 限界上下文 | 供应商上下文 |
| 数据主权 | 供应商系统拥有供应商协同事实、供货关系协同、供应商报价、订单确认、ASN、质量整改、供应商评分、对账确认、退供协同状态 |
...
| 核心原则 | 供应商系统不直接修改采购订单、库存余额或财务账，只通过事件表达协同事实 |
...
| 不泄露内部表结构 | 事件载荷不直接等同数据库表字段，避免下游依赖供应商系统内部实现 |
...
`sup_domain_event` 是供应商系统的 Outbox 表。聚合命令成功后，应用服务在业务事务内写入。
...
`sup_event_consume_log` 是供应商系统消费外部事件的 Inbox/幂等表。唯一键为 `source_system + event_code + consumer_name`。
...
3. 校验供应商存在且未禁用，事件中的 `supplierId` 与供应商系统可协同主体一致。
...
| 场景 | 风险 | 处理方式 |
| --- | --- | --- |
| 供应商用户重复点击确认 | 重复反馈采购系统 | 使用 `X-Idempotency-Key`；命中后返回原确认结果，不重复写 `sup_domain_event` |
...
供应商系统的事件生产与消费核心是“协同事实可追溯”：供应商确认了什么、发了什么货、提交了什么整改、反馈了什么对账差异、签收了什么退供，都必须变成可追踪的领域事件。
