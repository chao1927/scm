...
> 本文根据 [OMS领域模型](../03-核心业务模型/05-OMS领域模型/01-OMS领域模型.md)、[05-OMS系统产品功能设计](../04-子系统功能设计/05-OMS系统/01-OMS系统产品功能设计.md)、[05-OMS系统数据库设计](../05-子系统数据库设计/05-OMS系统数据库设计.md)、[05-OMS系统接口设计](../06-子系统接口设计/05-OMS系统接口设计.md) 和 [上下文映射与领域事件目录](../06-子系统接口设计/00-上下文映射与领域事件目录.md) 整理。本文专门说明 OMS 在聚合、领域服务、应用服务执行命令后如何生产事件，消费外部事件后如何改变本地订单履约数据，事件包含哪些字段属性，以及事件如何落表、发布、重试和审计。
...
| TMS | `TransportSigned` | TMS事件消费服务 | 销售订单已签收，满足完成条件后订单完成 | `SalesOrderSigned`、`SalesOrderCompleted` |
...
| 项 | 设计 |
| --- | --- |
| 来源事件 | `TransportSigned` |
| 来源上下文 | TMS/物流上下文 |
| 消费者 | `OmsTransportSignedConsumer` |
| 应用服务 | TMS事件消费服务、订单状态汇总服务 |
| 消费日志 | `oms_event_consume_log` |
...
| 消费 | `TransportSigned`、`TransportRejected`、`TrackingAppended`、`LogisticsExceptionRegistered` | TMS 事件消费服务 | `oms_event_consume_log`、`oms_outbound`、`oms_fulfillment`、`oms_sales_order`、`oms_after_sale` |
...
| 方向 | 事件 | 触发或消费逻辑 | 数据变化 |
| --- | --- | --- | --- |
| 生产 | `SalesDeliveryRequested` | 履约单创建或出库指令下发后请求配送 | 履约配送状态进入待运输，TMS 创建销售发货运输任务 |
| 生产 | `ReturnPickupRequested` | 售后审核通过且需要上门取件 | 售后单记录取件任务状态，TMS 创建退货取件任务 |
| 消费 | `WaybillCreated` | TMS 创建销售或退货运单 | OMS 订单/售后详情展示运单、承运商、预计到达 |
| 消费 | `TrackingAppended` | TMS 追加轨迹 | OMS 物流时间线、客服查询读模型刷新 |
| 消费 | `TransportSigned` | 客户签收销售包裹或退货到仓签收 | 履约签收读模型更新；退货仍等待 WMS 收货质检 |
| 消费 | `TransportRejected` | 客户拒收或退货被拒收 | 生成客服待办，进入补发、退款、重新配送或异常关闭判断 |
| 消费 | `LogisticsExceptionRegistered` / `LogisticsExceptionClosed` | 延误、破损、丢失、拒收、异常关闭 | 客服处理台和售后单时间线更新 |
