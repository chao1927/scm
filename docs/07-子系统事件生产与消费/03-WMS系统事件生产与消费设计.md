...
> 本文根据 [WMS领域模型](../03-核心业务模型/03-WMS领域模型/01-WMS领域模型.md)、[03-WMS系统产品功能设计](../04-子系统功能设计/03-WMS系统/01-WMS系统产品功能设计.md)、[03-WMS系统数据库设计](../05-子系统数据库设计/03-WMS系统数据库设计.md)、[03-WMS系统接口设计](../06-子系统接口设计/03-WMS系统接口设计.md) 和 [上下文映射与领域事件目录](../06-子系统接口设计/00-上下文映射与领域事件目录.md) 整理。本文专门说明 WMS 在聚合、领域服务、应用服务执行命令后如何生产事件，消费外部事件后如何改变本地作业数据，事件包含哪些字段属性，以及事件如何落表、发布、重试和审计。
...
| 一致性 | 单个作业聚合内部强一致；WMS 与采购、OMS、库存、BMS、01-供应商系统通过事件最终一致 |
...
| 方向 | 事件 | 触发或消费逻辑 | 数据变化 |
| --- | --- | --- | --- |
| 生产 | `PackageCompleted` | 复核包装完成后向 TMS 提供包裹、重量、体积 | `wms_package` 面单状态待生成，TMS 可生成运单/面单/费用计量 |
| 生产 | `OutboundOrderShipped` | 发货交接完成，仓库把包裹交给承运商 | 出库单已发货，TMS 运单可推进已发运/已揽收 |
| 生产 | `ReturnInboundReceived` / `InboundOrderReceived` | 采购、售后、调拨到货后 WMS 收货 | 中央库存以 WMS 上架事件入账，TMS 到达只作提示 |
| 消费 | `ShippingLabelGenerated` / `ShippingLabelVoided` | TMS 生成或作废面单 | 更新包裹面单状态，控制打印/补打/作废 |
| 消费 | `TransportArrived` | TMS 回传到达仓库 | 入库到货页面生成提示，不直接完成收货 |
| 消费 | `LogisticsExceptionRegistered` | TMS 回传运输异常 | 入库、出库、调拨页面展示异常标记，必要时创建仓内异常 |
