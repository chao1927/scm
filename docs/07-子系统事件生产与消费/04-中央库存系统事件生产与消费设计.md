...
> 本文根据 [中央库存领域模型](../03-核心业务模型/04-中央库存领域模型/01-中央库存领域模型.md)、[04-中央库存系统产品功能设计](../04-子系统功能设计/04-中央库存系统/01-中央库存系统产品功能设计.md)、[04-中央库存系统数据库设计](../05-子系统数据库设计/04-中央库存系统数据库设计.md)、[04-中央库存系统接口设计](../06-子系统接口设计/04-中央库存系统接口设计.md) 和 [上下文映射与领域事件目录](../06-子系统接口设计/00-上下文映射与领域事件目录.md) 整理。本文专门说明04-中央库存系统在聚合、领域服务、应用服务执行命令后如何生产事件，消费外部事件后如何改变库存账本，事件包含哪些字段属性，以及事件如何落表、发布、重试和审计。
...
| 外部事实主权 | WMS 拥有仓内实物事实，OMS 拥有履约取消和销售意图，主数据拥有 SKU/仓库/货主维度，09-权限系统拥有审批事实 |
...
8. 外部事件由 RocketMQ Consumer 接收后先写 `inv_event_consume_log`，再由消费应用服务处理，并写 `inv_stock_event_log`；不提供 HTTP 事件入口。
...
| 方向 | 事件 | 触发或消费逻辑 | 数据变化 |
| --- | --- | --- | --- |
| 消费 | `TransportShipped` | 调拨、退货、退供等运输发运 | 更新在途读模型和库存追踪视图；库存余额以 WMS 出库事实为准 |
| 消费 | `TransportArrived` | TMS 回传到达调入仓、退货仓或供应商 | 标记在途到达提醒，不增加可用库存 |
| 消费 | `TransportSigned` | TMS 回传签收 | 调拨/退货/退供追踪视图更新，等待 WMS 入库或业务补偿 |
| 消费 | `TransportRejected` | TMS 回传拒收 | 在途异常视图标记拒收，等待 OMS/采购/WMS 补偿命令 |
| 消费 | `LogisticsExceptionRegistered` / `LogisticsExceptionClosed` | 延误、破损、丢失、拒收等运输异常 | 库存异常看板更新，不直接改 `inv_stock_balance` |
...
## 12. 调拨事件收口（DOC-NEXT-001）

| 事件 | 事实拥有者 | 最小载荷 | 消费规则 |
| --- | --- | --- | --- |
| `TransferOutboundCompleted` | WMS | eventId、调拨号、源仓、SKU/批次、出库量、时间 | 关闭预占并扣减源仓；重复不记账 |
| `TransferInTransit` | TMS | eventId、调拨号、运输任务/运单、启运时间 | 只推进运输阶段，不改余额 |
| `TransferReceived` | WMS | eventId、调拨号、目标仓、本次/累计实收、finalReceipt、质量处置 | 上架/处置完成后记目标仓；最终计算差异 |
| `TransferDifferenceRaised` | 中央库存 | 调拨号、出库/实收/差异量、差异类型 | 创建差异待办，不伪造实收 |
| `TransferDifferenceConfirmed` | 中央库存 | 调拨号、确认差异、责任方、原因、凭证 | 锁定数量守恒并刷新读模型 |

所有调拨事件使用 `sourceSystem + eventId` 作为 Inbox 唯一键，并按调拨号保持业务顺序。乱序事实保留失败/待重放状态；禁止以生产方聚合版本替代本地版本。
