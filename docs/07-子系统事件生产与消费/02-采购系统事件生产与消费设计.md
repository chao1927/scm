...
> 本文根据 [02-采购系统领域模型](../03-核心业务模型/02-采购领域模型/01-采购系统领域模型.md)、[02-采购系统产品功能设计](../04-子系统功能设计/02-采购系统/01-采购系统产品功能设计.md)、[02-采购系统数据库设计](../05-子系统数据库设计/02-采购系统数据库设计.md)、[02-采购系统接口设计](../06-子系统接口设计/02-采购系统接口设计.md) 和 [上下文映射与领域事件目录](../06-子系统接口设计/00-上下文映射与领域事件目录.md) 整理。本文专门说明02-采购系统在执行命令后如何生产事件、消费外部事件时如何修改本地数据、事件载荷包含哪些信息，以及事件如何落表、发布、重试和审计。
...
| 事件消费 | 02-采购系统消费主数据、供应商系统、WMS、中央库存、预算/审批等上下文发布的事件 |
...
| --- | --- |
| 限界上下文 | 采购上下文 |
| 数据主权 | 02-采购系统拥有采购申请、询价、报价、比价、价格、采购订单、订单变更、入库跟踪快照、退供申请等采购侧事实 |
...
flowchart LR
    ...
    Broker --> Downstream["01-供应商系统/WMS/BMS/BI"]
```
...
`pur_domain_event` 是02-采购系统的 Outbox 表。应用服务在业务事务内写入该表，事件发布任务异步投递。
...
`pur_event_consume_log` 是02-采购系统消费外部事件的 Inbox/幂等表。事件入口先尝试写入消费日志，唯一键为 `source_system + event_code + consumer_name`。
...
| `SupplierSkuEnabled` | 01-供应商系统/主数据 | 供应商商品事件消费服务 | 询价候选供应商 | 更新供应商可供 SKU 关系 | `SUPPLIER:{eventId}:{supplierSkuId}` |
...
| 方向 | 事件 | 触发或消费逻辑 | 数据变化 |
| --- | --- | --- | --- |
| 生产 | `AsnSubmitted` | ASN 提交后作为采购到货运输需求输入 | 采购保留 ASN 与运输任务关联状态为待创建 |
| 生产 | `SupplierReturnApproved` | 退供应商单审核后可请求 TMS 创建退供运输任务 | 退供单状态进入待出库/待运输 |
| 消费 | `TransportArrived` | TMS 回传采购到货或调拨到达 | 更新采购到货读模型和仓库到货提醒，不关闭采购订单 |
| 消费 | `TransportSigned` | TMS 回传退供供应商签收或采购到货签收 | 更新退供签收状态；采购入库仍以 WMS 上架为准 |
| 消费 | `TransportRejected` | 供应商拒收退供或采购运输拒收 | 生成采购物流异常待办，等待退回、索赔或重新发运 |
| 消费 | `LogisticsExceptionRegistered` / `LogisticsExceptionClosed` | 运输延误、破损、丢失、拒收、费用争议 | 更新采购异常看板，补充索赔和退供处理依据 |
| 消费 | `LogisticsFeeSourceGenerated` | TMS 生成采购/退供运费来源 | 采购订单、退供单详情展示费用来源号，最终费用归 BMS |

## 11. 设计结论

02-采购系统的事件设计应围绕“采购意图、供应商承诺、采购执行进度”展开。02-采购系统自己生产的事件，来自聚合命令成功后的业务事实；外部系统发来的事件，必须先进入消费日志，再由采购应用服务和聚合处理，不能直接改表。
