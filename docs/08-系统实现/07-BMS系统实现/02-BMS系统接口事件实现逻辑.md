...
## 10. 退款实现收口（DOC-NEXT-001）

退款由独立[退款结算聚合](../../03-核心业务模型/07-BMS领域模型/11-退款结算聚合CQRS设计.md)管理。创建用例锁定账单/额度行并校验累计；支付回执先以全局 `receiptNo` 声明，再在同一事务推进聚合、刷新累计投影并写 Outbox。

现有实现已覆盖 REQUESTED、FINISHED、FAILED、失败重试和回执唯一性；`BMS-NEXT-002` 仍需补 `CONFIRMATION_PENDING/CLOSED`、主动查单、人工双人复核、回执金额币种商户号校验及创建请求幂等持久化。
