...
| 用例 | 必须实现 | 当前差异 |
| --- | --- | --- |
| 创建退款 | 锁账单/额度行、累计不超额、请求幂等、金额币种快照 | 已有累计校验；请求幂等持久化待补 |
| 支付回执 | 验签、防重放、回执全局唯一、金额/币种/商户号匹配 | 已有回执唯一；业务字段匹配待补 |
| 超时待确认 | 进入 `CONFIRMATION_PENDING`、继续占额、主动查单 | 待实现 |
| 失败重试 | 仅 FAILED，重新占额并生成新 attempt | 已有状态重试；attempt/重新占额证据待补 |
| 人工处置 | 待确认状态、凭证、高风险权限、双人复核 | 待实现 |

退款状态、回执记录、累计投影、审计和 Outbox 必须同事务；迟到/冲突回执进入人工待办，不覆盖终态。
