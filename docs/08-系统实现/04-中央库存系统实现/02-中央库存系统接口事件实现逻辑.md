...
## 10. 调拨实现收口（DOC-NEXT-001）

`StockTransferApplicationService` 以[调拨单聚合](../../03-核心业务模型/04-中央库存领域模型/08-调拨单聚合CQRS设计.md)为写模型入口，库存账户/预占聚合负责记账。出库事实必须在同一用例关闭预占并扣减源仓；入库事实仅在 WMS 上架/处置完成后增加目标仓。

实现门禁：创建幂等参数一致性、事件 Inbox 去重、乱序待重放、多收拒绝、差异责任证据、双仓写权限和 Outbox 原子性。现有代码已有调拨主链路，但这些门禁需由 `INV-NEXT-002` 逐项回归确认。
