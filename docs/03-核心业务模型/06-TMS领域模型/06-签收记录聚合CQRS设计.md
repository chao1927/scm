...
签收记录聚合保存客户、供应商、调入仓或退货仓的签收、拒收、部分签收和签收异常事实，并通知业务系统推进订单、退供、调拨、退货或中央库存在途状态。
...
| 权限审批已通过 | 放行签收冲正或拒收改判等高危动作 | 审批事件号 + 运单号 + 操作类型 |
...
| 签收异常看板 | 处理签收冲突、拒收、部分签收，并展示对 OMS、采购、调拨和中央库存的影响 |
