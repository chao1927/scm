...
运输任务聚合承接 OMS、采购、供应商、WMS、调拨等系统的运输需求，统一校验来源单据、运输场景、起止地址、物流产品、费用责任和幂等键。涉及取消、改承运商、异常关闭时要校验权限系统的物流数据范围和审批结果。
...
| 改派承运商 | 物流专员 | 已分配未发运时允许改派；命中高风险策略需审批 | 承运商选择服务 | 运输任务已分配承运商 |
...
| 权限审批已通过 | 放行改派承运商、取消任务或异常关闭等高危动作 | 审批事件号 + 任务号 + 操作类型 |
| 权限审批已拒绝 | 标记高危动作被拒绝，不改变任务状态 | 审批事件号 + 任务号 + 操作类型 |
