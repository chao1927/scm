...
运单聚合管理承运商下单、运单号、运输状态、发运、揽收、到达、签收、拒收、作废和异常关闭。运单发运、到达、签收、拒收等事实会被 OMS、采购、WMS、中央库存和 BMS 消费，作废运单属于高危动作，需要权限、审批和审计闭环。
...
| 标记运单创建失败 | 系统 | 保存承运商失败原因和重试状态 | 运单幂等服务 | 运单创建失败 |
...
| 权限审批已通过 | 放行作废运单、强制作废或异常关闭动作 | 审批事件号 + 运单号 + 操作类型 |
...
| 运单详情 | 查看任务、包裹、面单、轨迹、签收、异常、费用来源、中央库存在途影响 |
