...
物流异常聚合处理运输延误、破损、丢失、拒收、承运商拒收、地址异常、费用争议等问题，形成业务补偿、中央库存在途异常占用和 BMS 索赔依据。
...
| 权限审批已通过 | 放行异常关闭、责任方改判或索赔口径修正 | 审批事件号 + 异常号 + 操作类型 |
...
| 异常影响单据页 | 查看影响的订单、采购、调拨、退供、售后、中央库存在途和 BMS 费用 |
