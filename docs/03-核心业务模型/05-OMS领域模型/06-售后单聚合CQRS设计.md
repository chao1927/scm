...
处理仅退款、退货退款、拒收退款、退货取件、换货补发等逆向权益，维护审核、退货运输、退货验收、退款、补发和关闭状态。
...
| 子域类型 | 核心域，售后逆向履约 |
| 聚合根 | 售后单 |
| 数据主权 | OMS 拥有 `售后单` 的售后权益、审核、退货/补发编排和领域事件；TMS 拥有退货取件、退货运单、补发运单和运输异常事实 |
| 主要使用角色 | 客户、客服、财务、WMS、TMS、BMS、订单运营 |
...
| 请求退货取件 | 客服/系统 | 退货退款或换货审核通过后请求 TMS 创建退货取件任务 | 退货物流协同服务 | 退货取件已请求 |
| 登记退货运单 | 客户/客服/TMS | 登记客户寄回运单或消费 TMS 退货运单事件 | 退货物流协同服务 | 退货运单已登记 |
...
| returnLogisticsRef | 退货物流引用 | 值对象 | 是 | 请求退货取件/登记退货运单 | 退货运单号、取件任务号、退货运输状态、到仓时间、异常原因 |
| reshipmentLogisticsRef | 补发物流引用 | 值对象 | 是 | 创建补发/消费补发运单 | 补发履约单、补发运单号、补发运输状态、签收时间 |
...
| 创建售后 | 客服 | 围绕售后单执行创建售后，校验订单、客户、SKU、金额、履约状态、幂等键和权限 | 售后权益校验服务 | 售后单已创建 |
| 审核售后 | 客服 | 围绕售后单执行审核售后，校验订单、客户、SKU、金额、履约状态、幂等键和权限 | 售后权益校验服务 | 售后已审核 |
| 请求退货取件 | 客服/系统 | 校验售后已审核、退货地址和退货仓有效，请求 TMS 创建取件任务 | 退货物流协同服务 | 退货取件已请求 |
| 登记退货运单 | 客户/客服/TMS | 登记客户寄回运单或绑定 TMS 退货运单，更新退货物流状态 | 退货物流协同服务 | 退货运单已登记 |
| 确认退货验收 | 客服 | 围绕售后单执行确认退货验收，校验订单、客户、SKU、金额、履约状态、幂等键和权限 | 售后权益校验服务 | 退货已验收 |
| 发起退款 | 客服 | 围绕售后单执行发起退款，校验订单、客户、SKU、金额、履约状态、幂等键和权限 | 售后权益校验服务 | 退款已请求 |
| 创建补发 | 客服 | 围绕售后单执行创建补发，校验订单、客户、SKU、金额、履约状态、幂等键和权限 | 售后权益校验服务 | 补发已请求 |
| 关闭售后 | 客服 | 围绕售后单执行关闭售后，校验订单、客户、SKU、金额、履约状态、幂等键和权限 | 售后权益校验服务 | 售后已完成 |
...
| 请求退货取件 | 售后已审核且售后类型需要退货，退货地址、退货仓、物流产品有效 | 记录取件请求并产生事件 退货取件已请求 | 取件失败进入客服待办；客户自行寄回则登记退货运单 |
| 登记退货运单 | 运单归属当前售后，未重复绑定其他售后 | 更新退货运单和运输状态，产生事件 退货运单已登记 | 运单冲突进入人工核对；重复事件幂等忽略 |
...
| 退货物流协同服务 | 管理退货取件、客户寄回、退货到仓和退货运输异常，确保退货物流状态与售后退款路径一致。 |
...
| 退货取件已请求 | 请求退货取件 | 售后单ID、订单号、退货地址、退货仓、物流产品、取件时段 | TMS、客服、读模型、审计日志 |
| 退货运单已登记 | 登记退货运单 | 售后单ID、订单号、退货运单号、承运商、退货运输状态 | WMS、TMS、读模型、审计日志 |
...
| TMS运输已拒收 | 售后物流事件处理器 | 按拒收规则创建或关联售后单，进入退款、补发或退回验收路径 | TMS上下文+事件编号+waybillNo |
| TMS退货已到达 | 售后物流事件处理器 | 更新退货运输到仓状态，等待 WMS 验收 | TMS上下文+事件编号+returnWaybillNo |
| TMS物流异常已登记 | 售后物流事件处理器 | 记录退货丢失、破损、取件失败或补发异常，生成客服待办 | TMS上下文+事件编号+exceptionNo |
...
sequenceDiagram
    ...
    AppService->>Aggregate: 执行命令 请求退货取件
    Aggregate->>EventLog: 产生事件 退货取件已请求
    EventLog->>ReadModel: 投影状态 待取件
...
| TMS边界 | TMS 退货到达不等于 WMS 验收通过；退货退款仍必须满足售后规则、免退规则或 WMS 验收事实 |
...
| 售后单详情读模型 | 详情页展示 | 单头、明细、状态历史、事件历史、操作日志、退货运单、补发运单、运输异常 |
| 履约链路追踪读模型 | 串起订单、履约、预占、出库、运单、发货、售后 | 订单号、履约单、预占号、出库单、运单号、售后单、当前节点 |
| 异常处理读模型 | 处理缺货、取消失败、仓库拒单、退货取件失败、退货丢损、退款失败 | 异常类型、责任人、处理状态、阻塞原因 |
...
- OMS 拥有订单履约编排状态；中央库存拥有库存数量账本；WMS 拥有仓内作业事实；TMS 拥有退货和补发运输事实；BMS 拥有退款和入账事实。
