...
OMS 向 WMS 下发仓库出库指令，并跟踪 WMS 接单、拣货、发货、取消和异常。
...
| 子域类型 | 核心域，仓库出库指令 |
| 聚合根 | 出库单 |
| 数据主权 | OMS 拥有 `出库单` 的订单履约编排状态、出库指令、取消售后入口和领域事件；库存、仓内作业、退款入账由对应上下文拥有 |
| 主要使用角色 | 订单运营、WMS、仓配运营、中央库存 |
...
| 出库状态汇总服务 | 围绕出库单的订单状态、履约约束、客户权益、库存结果、WMS事实和规则配置进行业务判定。 |
...
sequenceDiagram
    ...
    Aggregate->>AppService: 创建可下发WMS的出库单
...
| 出库单详情读模型 | 详情页展示 | 单头、明细、状态历史、事件历史、操作日志 |
