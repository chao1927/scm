...
处理客户、渠道或客服发起的取消请求，判断是否可取消、是否需要 WMS 拦截、是否释放库存或转售后。
...
| 子域类型 | 核心域，取消与拦截 |
| 聚合根 | 取消申请 |
| 数据主权 | OMS 拥有 `取消申请` 的订单履约编排状态、出库指令、取消售后入口和领域事件；库存、仓内作业、退款入账由对应上下文拥有 |
| 主要使用角色 | 客户、客服、渠道系统、WMS、中央库存 |
...
sequenceDiagram
    ...
    MessageBroker->>EventConsumer: 投递外部事件 取消请求已提交
    ...
    Aggregate->>AppService: 创建取消申请并判断可取消路径
...
| 取消申请详情读模型 | 详情页展示 | 单头、明细、状态历史、事件历史、操作日志 |
