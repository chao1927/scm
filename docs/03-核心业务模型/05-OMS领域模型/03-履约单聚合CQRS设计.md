...
依据订单、仓库、库存和物流策略生成履约执行单元，管理分仓、拆单、库存预占引用和履约状态。
...
| 子域类型 | 核心域，分仓履约编排 |
| 聚合根 | 履约单 |
| 数据主权 | OMS 拥有 `履约单` 的订单履约编排状态、出库指令、取消售后入口和领域事件；库存、仓内作业、退款入账由对应上下文拥有 |
| 主要使用角色 | 订单运营、系统分仓任务、中央库存、WMS |
...
| 履约状态汇总服务 | 围绕履约单的订单状态、履约约束、客户权益、库存结果、WMS事实和规则配置进行业务判定。 |
...
sequenceDiagram
    ...
    MessageBroker->>EventConsumer: 投递外部事件 订单已审核
    ...
    Aggregate->>AppService: 生成履约单并请求库存预占
...
| 履约单详情读模型 | 详情页展示 | 单头、明细、状态历史、事件历史、操作日志 |
