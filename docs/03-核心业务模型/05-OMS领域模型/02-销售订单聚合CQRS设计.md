...
承载客户购买意图，维护订单接入、审单、金额、客户、地址、履约、发货、签收、完成和取消主状态。
...
| 子域类型 | 核心域，订单接入与主状态 |
| 聚合根 | 销售订单 |
| 数据主权 | OMS 拥有 `销售订单` 的订单履约编排状态、出库指令、取消售后入口和领域事件；库存、仓内作业、退款入账由对应上下文拥有 |
| 主要使用角色 | 渠道系统、客服、订单运营、风控、客户 |
...
| 确认签收 | 渠道系统 | 围绕销售订单执行确认签收，校验订单、客户、SKU、金额、履约状态、幂等键和权限 | 订单幂等接入服务 | 订单已签收 |
...
| 订单状态汇总服务 | 围绕销售订单的订单状态、履约约束、客户权益、库存结果、WMS事实和规则配置进行业务判定。 |
...
| 订单已签收 | 确认签收 | 销售订单ID、订单号、渠道、客户、SKU、数量、金额、状态 | 中央库存、WMS、BMS、读模型、审计日志、报表 |
...
sequenceDiagram
    ...
    Aggregate->>AppService: 记录发货事实并推进订单状态
...
| 销售订单详情读模型 | 详情页展示 | 单头、明细、状态历史、事件历史、操作日志 |
