...
保存每次启用或关键变更后的不可变快照，支撑子系统引用、版本对比、事件发布和历史追溯；TMS 引用物流商、物流产品、地址区域、禁运规则、面单模板和轨迹接口配置时必须保存主数据版本快照。
...
| tmsSnapshotRequirement | TMS快照要求 | 值对象 | 是 | 生成版本、发布版本 | 标识 TMS 必须缓存的物流类主数据字段、版本号、生效时间和兼容策略 |
...
| TMS版本兼容校验服务 | 校验物流类主数据版本是否兼容 TMS 当前缓存和在途运单，关键字段变更是否需要灰度、生效时间或冻结新业务。 |

## 7. 事件产生逻辑

| 领域事件 | 触发命令 | 关键载荷 | 主要消费者 |
| --- | --- | --- | --- |
| 主数据版本已生成 | 生成版本 | 聚合ID、类型编码、主数据编码、版本号、状态`待发布`、变更摘要、目标系统、TMS快照要求、操作者、幂等键 | 采购、OMS、WMS、中央库存、TMS、BMS、供应商系统、读模型、审计日志 |
| 主数据版本已发布 | 发布版本 | 聚合ID、类型编码、主数据编码、版本号、状态`已发布`、变更摘要、目标系统、TMS快照要求、操作者、幂等键 | 采购、OMS、WMS、中央库存、TMS、BMS、供应商系统、读模型、审计日志 |
...
| 类型 | 设计 |
| --- | --- |
| 异常 | 快照缺失、版本号跳号、重复发布、发布载荷与版本不一致、历史版本被覆盖、TMS 在途运单仍引用旧版本但新版本缺少兼容策略。 |
...
| TMS主数据版本读模型 | 查询物流类主数据版本、生效时间、兼容策略和 TMS 发布状态 | 版本事件 + 发布日志 | 事件投影更新 |
...
关键假设：主数据系统拥有基础资料权威口径；TMS 引用物流类主数据时保存版本快照，在途运单按创建时快照执行。
