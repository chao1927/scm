...
支撑批量导入主数据，记录文件、导入模式、校验结果、行级错误、成功数量、失败数量和错误文件；物流商、物流产品、地址区域、禁运规则、面单模板和轨迹接口配置常通过批量导入初始化，必须先按字段模板和引用关系校验。
...
| 创建物流主数据导入任务 | 主数据专员 | 批量导入物流商、物流产品、地址区域、禁运规则、面单模板或轨迹接口配置，校验引用和 TMS 必填字段 | 物流导入校验服务 | 导入任务已创建 |
...
| logisticsImportProfile | 物流导入配置 | 值对象 | 是 | 创建导入任务、校验导入文件 | 标识导入物流类主数据、字段模板版本、引用校验范围、TMS 联调要求和错误文件格式 |
...
| 物流导入校验服务 | 校验物流类导入行的承运商编码、物流产品、服务区域、地址区域、禁运规则、面单模板和轨迹接口引用是否完整且不冲突。 |
...
| 类型 | 设计 |
| --- | --- |
| 异常 | 文件格式错误、字段缺失、重复行、引用不存在、编码冲突、部分成功、错误文件生成失败、物流产品引用不存在、服务区域与禁运区域冲突、面单模板字段缺失。 |
...
| 物流主数据导入读模型 | 查看物流商、物流产品、地址区域、禁运规则、面单模板、轨迹接口配置导入结果和错误行 | 导入任务事件 + 物流导入配置 | 事件投影更新 |
...
关键假设：主数据系统拥有基础资料权威口径；物流类主数据可批量导入，但启用和发布仍必须经过模板校验、审核、版本和 TMS 回执链路。
