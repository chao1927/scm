...
管理接入权限体系的业务系统、SSO 客户端、回调地址、首页地址、菜单根和应用状态，为子系统登录、授权和权限点注册提供边界。
...
| 配置SSO客户端 | 应用处于允许状态，授权人有管理权限，权限范围合法，命令未重复 | 修改应用状态为`已启用`，刷新权限版本或安全快照，产生`SSO客户端已配置` | 状态不匹配则拒绝；越权授权拦截；缓存失效后要求重新拉取权限 |
