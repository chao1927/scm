...
将账单、发票和结算金额交接给财务系统，记录凭证、入账结果、失败原因和关闭状态。
...
| --- | --- | --- | --- |
| 财务交接已创建 | 创建财务交接 | 聚合ID、单号、计费对象、账期、费用类型、方向、金额、税额、状态`待交接`、规则版本、来源事件、幂等键 | BMS读模型、对账、账单、发票交接、财务交接、审计日志、报表 |
...
| --- | --- | --- | --- |
| 账单已确认 | 财务交接应用服务 | 创建待财务交接任务 | 来源上下文+事件编号+业务主键 |
...
| 类型 | 设计 |
| --- | --- |
| 异常 | 账单未确认、发票缺失、凭证创建失败、财务回调重复、入账金额不一致、交接超时。 |
