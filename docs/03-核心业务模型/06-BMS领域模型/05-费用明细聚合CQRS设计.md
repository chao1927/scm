...
把可计费来源事实按照计费对象和规则版本计算为费用明细，维护确认、重算、作废和金额快照；物流费用明细需要消费 TMS 的运单费用来源、签收/拒收结果和物流异常责任，形成可追溯的运输成本或索赔费用。
...
| 计算物流费用 | BMS系统 | 根据运单费用来源、承运商、物流产品、线路、重量体积、签收或拒收结果匹配物流计费规则，生成运费、取件费、退回费、赔付或索赔明细 | 物流费用计算服务 | 费用明细已生成 |
...
| waybillFeeSnapshot | 运单费用快照 | 值对象 | 是 | 计算费用、重算费用 | 保存运单号、承运商、物流产品、线路、重量、体积、件数、签收/拒收结果、异常责任和 TMS 费用来源事件号 |
...
| 计算物流费用 | BMS系统 | 加载费用明细聚合，校验 TMS 运单费用来源、签收/拒收结果、承运商计费对象、物流计价规则版本和账期，生成物流费用行并写入事件发布表 | 物流费用计算服务 | 费用明细已生成 |
...
| 计算物流费用 | 运单费用来源未重复计费，承运商计费对象有效，物流计价规则已发布，签收/拒收/异常责任满足计费条件 | 修改费用明细状态为`待对账`，写入运单费用快照和物流费用行，产生`费用明细已生成` | TMS 来源修正后未确认费用可重算；已确认费用必须生成调整单或差异待办 |
...
| 物流费用计算服务 | 根据 TMS 运单费用来源、签收/拒收结果、异常责任、线路、重量体积和物流计价规则计算物流费用，保证 TMS 原始事实只作为来源证据，BMS 金额必须由本地规则和调整链路确认。 |

## 7. 事件产生逻辑

| 领域事件 | 触发命令 | 关键载荷 | 主要消费者 |
| --- | --- | --- | --- |
| 费用明细已生成 | 计算费用、计算物流费用 | 聚合ID、单号、计费对象、账期、费用类型、方向、金额、税额、状态`待对账`、规则版本、来源事件、运单费用快照、幂等键 | BMS读模型、对账、账单、发票交接、财务交接、审计日志、报表 |
...
| 物流费用来源已采集 | 费用计算应用服务 | 匹配承运商计费对象和物流计价规则，生成物流费用明细或索赔费用明细 | TMS+运单号+费用项+账期 |
...
| 类型 | 设计 |
| --- | --- |
| 异常 | 无可用规则、规则冲突、金额为负、税额不平、来源事实撤销、已确认后要求重算、运单重量体积缺失、TMS 费用来源被修正、签收/拒收结果与计费条件不匹配。 |
...
| 物流费用明细读模型 | 按运单号、承运商、物流产品、线路、签收/拒收结果和异常责任查询物流费用 | 费用明细事件 + TMS 来源快照 | 事件投影更新 |
...
关键假设：BMS 消费业务事实并生成费用和账单，TMS 拥有运单、签收、异常和原始物流费用来源，财务系统拥有最终凭证和资金入账事实。
