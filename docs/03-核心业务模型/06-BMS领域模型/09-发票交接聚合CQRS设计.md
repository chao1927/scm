...
围绕账单发起开票、回填发票、作废发票和同步发票状态，保证发票金额不超过账单口径。
...
| --- | --- | --- | --- |
| 发票交接已创建 | 创建开票交接 | 聚合ID、单号、计费对象、账期、费用类型、方向、金额、税额、状态`待开票`、规则版本、来源事件、幂等键 | BMS读模型、对账、账单、发票交接、财务交接、审计日志、报表 |
...
| --- | --- | --- | --- |
| 账单已确认 | 发票交接应用服务 | 创建可开票交接任务 | 来源上下文+事件编号+业务主键 |
...
| 类型 | 设计 |
| --- | --- |
| 异常 | 发票抬头缺失、税号错误、开票金额超额、部分开票、发票作废、开票系统回调重复或失败。 |
