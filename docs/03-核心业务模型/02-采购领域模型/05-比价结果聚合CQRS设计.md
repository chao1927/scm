...
在报价、采购价格、交期、供应商评分、质量表现之间做采购决策，并留下可审计的推荐和定标依据。
...
| 子域类型 | 核心域，决定向谁采购 |
| 聚合根 | 比价结果 |
| 数据主权 | 采购上下文拥有 `比价结果` 的生命周期、状态、业务规则和领域事件；外部系统只能通过命令或事件协作 |
...
| --- | --- | --- | --- |
| 生成比价 | 比价结果状态允许执行，来源数据和权限有效 | 修改比价结果状态或明细并产生事件 比价已生成 | 状态不匹配则拒绝；外部协作失败进入待办或补偿流程 |
| 推荐供应商 | 比价结果状态允许执行，来源数据和权限有效 | 修改比价结果状态或明细并产生事件 供应商已推荐 | 状态不匹配则拒绝；外部协作失败进入待办或补偿流程 |
...
| 领域服务 | 核心逻辑 |
| --- | --- |
| 比价评分计算服务 | 围绕比价结果的状态、不变量、外部事实快照和策略配置进行业务判定。 |
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 比价结果列表读模型 | 查询、分页、筛选 | 单号、状态、供应商、金额、数量、更新时间 |
| 比价结果详情读模型 | 详情页展示 | 单头、明细、状态历史、事件历史、操作日志 |
| 比价结果异常读模型 | 异常处理看板 | 异常类型、责任人、处理状态、阻塞原因 |
