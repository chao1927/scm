...
维护采购可用价格、价格版本、生效期、税率和币种，确保采购订单取价有明确依据。
...
| 子域类型 | 核心域，控制采购价格主权 |
| 聚合根 | 采购价格 |
| 数据主权 | 采购上下文拥有 `采购价格` 的生命周期、状态、业务规则和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | 采购员、采购经理、价格管理员 |
...
| --- | --- | --- | --- | --- |
| 创建采购价格 | 采购经理 | 围绕采购价格执行创建采购价格，校验状态、来源和业务规则 | 价格冲突检测服务 | 采购价格已创建 |
| 提交价格审批 | 采购经理 | 围绕采购价格执行提交价格审批，校验状态、来源和业务规则 | 价格冲突检测服务 | 采购价格已提交审批 |
| 生效采购价格 | 采购经理 | 围绕采购价格执行生效采购价格，校验状态、来源和业务规则 | 价格冲突检测服务 | 采购价格已生效 |
| 失效采购价格 | 采购经理 | 围绕采购价格执行失效采购价格，校验状态、来源和业务规则 | 价格冲突检测服务 | 采购价格已失效 |
...
| --- | --- | --- | --- | --- | --- |
| 采购价格Id | 采购价格ID | 聚合根 | 否 | 创建采购价格 | 全局唯一 |
| 采购价格No | 采购价格单号 | 值对象 | 否 | 创建采购价格 | 按编码规则生成 |
| status | 业务状态 | 值对象 | 是 | 状态推进命令 | 必须按状态机流转 |
| lineList | 明细行 | 内部实体集合 | 是 | 创建或变更命令 | 记录SKU、数量、价格、交期等明细 |
| sourceRef | 来源引用 | 值对象 | 否 | 创建采购价格 | 记录请购、询价、报价、PO或外部事件来源 |
...
4. 通过资源库加载 `采购价格` 聚合根，新建场景先校验业务唯一性。
...
| --- | --- | --- | --- |
| 创建采购价格 | 采购价格状态允许执行，来源数据和权限有效 | 修改采购价格状态或明细并产生事件 采购价格已创建 | 状态不匹配则拒绝；外部协作失败进入待办或补偿流程 |
| 提交价格审批 | 采购价格状态允许执行，来源数据和权限有效 | 修改采购价格状态或明细并产生事件 采购价格已提交审批 | 状态不匹配则拒绝；外部协作失败进入待办或补偿流程 |
| 生效采购价格 | 采购价格状态允许执行，来源数据和权限有效 | 修改采购价格状态或明细并产生事件 采购价格已生效 | 状态不匹配则拒绝；外部协作失败进入待办或补偿流程 |

## 6. 领域服务逻辑

| 领域服务 | 核心逻辑 |
| --- | --- |
| 价格冲突检测服务 | 围绕采购价格的状态、不变量、外部事实快照和策略配置进行业务判定。 |
| 价格生效策略服务 | 围绕采购价格的状态、不变量、外部事实快照和策略配置进行业务判定。 |
| 价格失效判定服务 | 围绕采购价格的状态、不变量、外部事实快照和策略配置进行业务判定。 |

## 7. 事件产生逻辑

| 领域事件 | 触发命令 | 关键载荷 | 主要消费者 |
| --- | --- | --- | --- |
| 采购价格已创建 | 创建采购价格 | 采购价格ID、业务状态、关键明细摘要 | 本领域读模型、上下游协作系统、审计日志 |
| 采购价格已提交审批 | 提交价格审批 | 采购价格ID、业务状态、关键明细摘要 | 本领域读模型、上下游协作系统、审计日志 |
| 采购价格已生效 | 生效采购价格 | 采购价格ID、业务状态、关键明细摘要 | 本领域读模型、上下游协作系统、审计日志 |
| 采购价格已失效 | 失效采购价格 | 采购价格ID、业务状态、关键明细摘要 | 本领域读模型、上下游协作系统、审计日志 |
...
| --- | --- | --- | --- |
| 比价已定标 | 外部事件消费服务 | 生成或更新采购价格版本 | 来源上下文+事件编号+业务主键 |
...
sequenceDiagram
    ...
    User->>CommandAPI: 采购经理提交命令 创建采购价格
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载采购价格聚合
...
sequenceDiagram
    ...
    User->>CommandAPI: 采购经理提交命令 提交价格审批
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载采购价格聚合
    Repository->>AppService: 返回聚合当前状态
    AppService->>DomainService: 执行业务规则校验
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 采购价格已提交审批
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 采购价格已提交审批
```
...
sequenceDiagram
    ...
    User->>CommandAPI: 采购经理提交命令 生效采购价格
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载采购价格聚合
    Repository->>AppService: 返回聚合当前状态
    AppService->>DomainService: 执行业务规则校验
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 采购价格已生效
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 采购价格已生效
```
...
sequenceDiagram
    ...
    MessageBroker->>EventConsumer: 投递外部事件 比价已定标
    ...
    Aggregate->>AppService: 生成或更新采购价格版本
...
sequenceDiagram
    ...
    AppService->>Aggregate: 执行命令 创建采购价格
    Aggregate->>EventLog: 产生事件 采购价格已创建
    EventLog->>ReadModel: 投影状态 草稿
    AppService->>Aggregate: 执行命令 提交价格审批
    Aggregate->>EventLog: 产生事件 采购价格已提交审批
    EventLog->>ReadModel: 投影状态 审批中
    AppService->>Aggregate: 执行命令 生效采购价格
    Aggregate->>EventLog: 产生事件 采购价格已生效
    EventLog->>ReadModel: 投影状态 已生效
    AppService->>Aggregate: 执行命令 失效采购价格
    Aggregate->>EventLog: 产生事件 采购价格已失效
    EventLog->>ReadModel: 投影状态 已失效
```
...
| 类型 | 规则 |
| --- | --- |
| 聚合不变量 | `采购价格` 的状态只能通过聚合根行为推进，内部实体不能被外部直接修改 |
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 采购价格列表读模型 | 查询、分页、筛选 | 单号、状态、供应商、金额、数量、更新时间 |
| 采购价格详情读模型 | 详情页展示 | 单头、明细、状态历史、事件历史、操作日志 |
| 采购价格异常读模型 | 异常处理看板 | 异常类型、责任人、处理状态、阻塞原因 |

## 12. 设计结论与待确认问题

### 12.1 设计结论

- `采购价格` 是采购领域内独立保护业务规则和状态流转的聚合根。
