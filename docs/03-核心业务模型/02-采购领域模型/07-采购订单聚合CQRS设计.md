...
形成对供应商的正式采购承诺，并跟踪审批、发布、供应商确认、收货、质检、上架、完成、取消和关闭。
...
| 子域类型 | 核心域，承载采购义务 |
| 聚合根 | 采购订单 |
| 数据主权 | 采购上下文拥有 `采购订单` 的生命周期、状态、业务规则和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | 采购员、采购经理、供应商系统、WMS、库存系统 |
...
| 采购执行进度判定服务 | 围绕采购订单的状态、不变量、外部事实快照和策略配置进行业务判定。 |
...
| 采购订单已发布 | 发布采购订单 | 采购订单ID、业务状态、关键明细摘要 | 本领域读模型、上下游协作系统、审计日志 |
...
sequenceDiagram
    ...
    MessageBroker->>EventConsumer: 投递外部事件 供应商订单已确认
    ...
    Aggregate->>AppService: 记录供应商确认结果并推进待到货
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 采购订单列表读模型 | 查询、分页、筛选 | 单号、状态、供应商、金额、数量、更新时间 |
| 采购订单详情读模型 | 详情页展示 | 单头、明细、状态历史、事件历史、操作日志 |
| 采购订单异常读模型 | 异常处理看板 | 异常类型、责任人、处理状态、阻塞原因 |
