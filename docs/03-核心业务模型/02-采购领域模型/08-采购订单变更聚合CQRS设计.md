...
管理采购订单数量、价格、交期、目的仓、取消和关闭等关键变化，避免直接覆盖历史采购承诺。
...
| 子域类型 | 核心域，保护采购承诺变更历史 |
| 聚合根 | 采购订单变更 |
| 数据主权 | 采购上下文拥有 `采购订单变更` 的生命周期、状态、业务规则和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | 采购员、采购经理、供应商系统 |
...
| --- | --- | --- | --- | --- |
| 创建订单变更 | 采购员 | 围绕采购订单变更执行创建订单变更，校验状态、来源和业务规则 | 订单变更可行性校验服务 | 采购订单变更已创建 |
| 提交订单变更 | 采购员 | 围绕采购订单变更执行提交订单变更，校验状态、来源和业务规则 | 订单变更可行性校验服务 | 采购订单变更已提交 |
| 审批订单变更 | 采购员 | 围绕采购订单变更执行审批订单变更，校验状态、来源和业务规则 | 订单变更可行性校验服务 | 采购订单变更已批准 |
| 生效订单变更 | 采购员 | 围绕采购订单变更执行生效订单变更，校验状态、来源和业务规则 | 订单变更可行性校验服务 | 采购订单变更已生效 |
| 作废订单变更 | 采购员 | 围绕采购订单变更执行作废订单变更，校验状态、来源和业务规则 | 订单变更可行性校验服务 | 采购订单变更已作废 |
...
| --- | --- | --- | --- | --- | --- |
| 采购订单变更Id | 采购订单变更ID | 聚合根 | 否 | 创建订单变更 | 全局唯一 |
| 采购订单变更No | 采购订单变更单号 | 值对象 | 否 | 创建订单变更 | 按编码规则生成 |
| status | 业务状态 | 值对象 | 是 | 状态推进命令 | 必须按状态机流转 |
| lineList | 明细行 | 内部实体集合 | 是 | 创建或变更命令 | 记录SKU、数量、价格、交期等明细 |
| sourceRef | 来源引用 | 值对象 | 否 | 创建订单变更 | 记录请购、询价、报价、PO或外部事件来源 |
...
4. 通过资源库加载 `采购订单变更` 聚合根，新建场景先校验业务唯一性。
...
| --- | --- | --- | --- |
| 创建订单变更 | 采购订单变更状态允许执行，来源数据和权限有效 | 修改采购订单变更状态或明细并产生事件 采购订单变更已创建 | 状态不匹配则拒绝；外部协作失败进入待办或补偿流程 |
| 提交订单变更 | 采购订单变更状态允许执行，来源数据和权限有效 | 修改采购订单变更状态或明细并产生事件 采购订单变更已提交 | 状态不匹配则拒绝；外部协作失败进入待办或补偿流程 |
| 审批订单变更 | 采购订单变更状态允许执行，来源数据和权限有效 | 修改采购订单变更状态或明细并产生事件 采购订单变更已批准 | 状态不匹配则拒绝；外部协作失败进入待办或补偿流程 |

## 6. 领域服务逻辑

| 领域服务 | 核心逻辑 |
| --- | --- |
| 订单变更可行性校验服务 | 围绕采购订单变更的状态、不变量、外部事实快照和策略配置进行业务判定。 |
| 订单变更差异判定服务 | 围绕采购订单变更的状态、不变量、外部事实快照和策略配置进行业务判定。 |
| 变更生效策略服务 | 围绕采购订单变更的状态、不变量、外部事实快照和策略配置进行业务判定。 |

## 7. 事件产生逻辑

| 领域事件 | 触发命令 | 关键载荷 | 主要消费者 |
| --- | --- | --- | --- |
| 采购订单变更已创建 | 创建订单变更 | 采购订单变更ID、业务状态、关键明细摘要 | 本领域读模型、上下游协作系统、审计日志 |
| 采购订单变更已提交 | 提交订单变更 | 采购订单变更ID、业务状态、关键明细摘要 | 本领域读模型、上下游协作系统、审计日志 |
| 采购订单变更已批准 | 审批订单变更 | 采购订单变更ID、业务状态、关键明细摘要 | 本领域读模型、上下游协作系统、审计日志 |
| 采购订单变更已生效 | 生效订单变更 | 采购订单变更ID、业务状态、关键明细摘要 | 本领域读模型、上下游协作系统、审计日志 |
| 采购订单变更已作废 | 作废订单变更 | 采购订单变更ID、业务状态、关键明细摘要 | 本领域读模型、上下游协作系统、审计日志 |
...
| --- | --- | --- | --- |
| 采购订单已发布 | 外部事件消费服务 | 建立可变更PO快照 | 来源上下文+事件编号+业务主键 |
...
sequenceDiagram
    ...
    User->>CommandAPI: 采购员提交命令 创建订单变更
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载采购订单变更聚合
...
sequenceDiagram
    ...
    User->>CommandAPI: 采购员提交命令 提交订单变更
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载采购订单变更聚合
    Repository->>AppService: 返回聚合当前状态
    AppService->>DomainService: 执行业务规则校验
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 采购订单变更已提交
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 采购订单变更已提交
```
...
sequenceDiagram
    ...
    User->>CommandAPI: 采购员提交命令 审批订单变更
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载采购订单变更聚合
    Repository->>AppService: 返回聚合当前状态
    AppService->>DomainService: 执行业务规则校验
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 采购订单变更已批准
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 采购订单变更已批准
```
...
sequenceDiagram
    ...
    Aggregate->>AppService: 建立可变更PO快照
...
sequenceDiagram
    ...
    AppService->>Aggregate: 执行命令 创建订单变更
    Aggregate->>EventLog: 产生事件 采购订单变更已创建
    EventLog->>ReadModel: 投影状态 草稿
    AppService->>Aggregate: 执行命令 提交订单变更
    Aggregate->>EventLog: 产生事件 采购订单变更已提交
    EventLog->>ReadModel: 投影状态 待审批
    AppService->>Aggregate: 执行命令 审批订单变更
    Aggregate->>EventLog: 产生事件 采购订单变更已批准
    EventLog->>ReadModel: 投影状态 已批准
    AppService->>Aggregate: 执行命令 生效订单变更
    Aggregate->>EventLog: 产生事件 采购订单变更已生效
    EventLog->>ReadModel: 投影状态 已生效
    AppService->>Aggregate: 执行命令 作废订单变更
    Aggregate->>EventLog: 产生事件 采购订单变更已作废
    EventLog->>ReadModel: 投影状态 已作废
```
...
| 类型 | 规则 |
| --- | --- |
| 聚合不变量 | `采购订单变更` 的状态只能通过聚合根行为推进，内部实体不能被外部直接修改 |
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 采购订单变更列表读模型 | 查询、分页、筛选 | 单号、状态、供应商、金额、数量、更新时间 |
| 采购订单变更详情读模型 | 详情页展示 | 单头、明细、状态历史、事件历史、操作日志 |
| 采购订单变更异常读模型 | 异常处理看板 | 异常类型、责任人、处理状态、阻塞原因 |

## 12. 设计结论与待确认问题

### 12.1 设计结论

- `采购订单变更` 是采购领域内独立保护业务规则和状态流转的聚合根。
