...
记录供应商针对询价或采购需求给出的价格、税率、币种、MOQ和交期承诺，作为比价和定标依据。
...
| 子域类型 | 核心域，承载供应商价格承诺 |
| 聚合根 | 供应商报价 |
| 数据主权 | 采购上下文拥有 `供应商报价` 的生命周期、状态、业务规则和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | 供应商业务员、采购员、采购经理 |
...
| --- | --- | --- | --- | --- |
| 录入报价 | 供应商业务员 | 围绕供应商报价执行录入报价，校验状态、来源和业务规则 | 报价有效性校验服务 | 供应商报价已创建 |
| 提交报价 | 供应商业务员 | 围绕供应商报价执行提交报价，校验状态、来源和业务规则 | 报价有效性校验服务 | 供应商报价已提交 |
| 确认报价 | 供应商业务员 | 围绕供应商报价执行确认报价，校验状态、来源和业务规则 | 报价有效性校验服务 | 供应商报价已确认 |
| 标记中标 | 供应商业务员 | 围绕供应商报价执行标记中标，校验状态、来源和业务规则 | 报价有效性校验服务 | 供应商报价已中标 |
| 标记未中标 | 供应商业务员 | 围绕供应商报价执行标记未中标，校验状态、来源和业务规则 | 报价有效性校验服务 | 供应商报价已失标 |
...
| --- | --- | --- | --- | --- | --- |
| 供应商报价Id | 供应商报价ID | 聚合根 | 否 | 录入报价 | 全局唯一 |
| 供应商报价No | 供应商报价单号 | 值对象 | 否 | 录入报价 | 按编码规则生成 |
| status | 业务状态 | 值对象 | 是 | 状态推进命令 | 必须按状态机流转 |
| lineList | 明细行 | 内部实体集合 | 是 | 创建或变更命令 | 记录SKU、数量、价格、交期等明细 |
| sourceRef | 来源引用 | 值对象 | 否 | 录入报价 | 记录请购、询价、报价、PO或外部事件来源 |
...
4. 通过资源库加载 `供应商报价` 聚合根，新建场景先校验业务唯一性。
...
| --- | --- | --- | --- |
| 录入报价 | 供应商报价状态允许执行，来源数据和权限有效 | 修改供应商报价状态或明细并产生事件 供应商报价已创建 | 状态不匹配则拒绝；外部协作失败进入待办或补偿流程 |
| 提交报价 | 供应商报价状态允许执行，来源数据和权限有效 | 修改供应商报价状态或明细并产生事件 供应商报价已提交 | 状态不匹配则拒绝；外部协作失败进入待办或补偿流程 |
| 确认报价 | 供应商报价状态允许执行，来源数据和权限有效 | 修改供应商报价状态或明细并产生事件 供应商报价已确认 | 状态不匹配则拒绝；外部协作失败进入待办或补偿流程 |

## 6. 领域服务逻辑

| 领域服务 | 核心逻辑 |
| --- | --- |
| 报价有效性校验服务 | 围绕供应商报价的状态、不变量、外部事实快照和策略配置进行业务判定。 |
| 价格与税率校验服务 | 围绕供应商报价的状态、不变量、外部事实快照和策略配置进行业务判定。 |
| 报价截止判定服务 | 围绕供应商报价的状态、不变量、外部事实快照和策略配置进行业务判定。 |

## 7. 事件产生逻辑

| 领域事件 | 触发命令 | 关键载荷 | 主要消费者 |
| --- | --- | --- | --- |
| 供应商报价已创建 | 录入报价 | 供应商报价ID、业务状态、关键明细摘要 | 本领域读模型、上下游协作系统、审计日志 |
| 供应商报价已提交 | 提交报价 | 供应商报价ID、业务状态、关键明细摘要 | 本领域读模型、上下游协作系统、审计日志 |
| 供应商报价已确认 | 确认报价 | 供应商报价ID、业务状态、关键明细摘要 | 本领域读模型、上下游协作系统、审计日志 |
| 供应商报价已中标 | 标记中标 | 供应商报价ID、业务状态、关键明细摘要 | 本领域读模型、上下游协作系统、审计日志 |
| 供应商报价已失标 | 标记未中标 | 供应商报价ID、业务状态、关键明细摘要 | 本领域读模型、上下游协作系统、审计日志 |
...
| --- | --- | --- | --- |
| 询价单已发布 | 外部事件消费服务 | 创建报价待办或报价草稿 | 来源上下文+事件编号+业务主键 |
...
sequenceDiagram
    ...
    User->>CommandAPI: 供应商业务员提交命令 录入报价
...
sequenceDiagram
    ...
    Aggregate->>AppService: 创建报价待办或报价草稿
...
sequenceDiagram
    ...
    AppService->>Aggregate: 执行命令 录入报价
    ...
    AppService->>Aggregate: 执行命令 标记中标
    Aggregate->>EventLog: 产生事件 供应商报价已中标
    EventLog->>ReadModel: 投影状态 中标
    AppService->>Aggregate: 执行命令 标记未中标
    Aggregate->>EventLog: 产生事件 供应商报价已失标
    EventLog->>ReadModel: 投影状态 未中标
```
...
| 类型 | 规则 |
| --- | --- |
| 聚合不变量 | `供应商报价` 的状态只能通过聚合根行为推进，内部实体不能被外部直接修改 |
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 供应商报价列表读模型 | 查询、分页、筛选 | 单号、状态、供应商、金额、数量、更新时间 |
| 供应商报价详情读模型 | 详情页展示 | 单头、明细、状态历史、事件历史、操作日志 |
| 供应商报价异常读模型 | 异常处理看板 | 异常类型、责任人、处理状态、阻塞原因 |

## 12. 设计结论与待确认问题

### 12.1 设计结论

- `供应商报价` 是采购领域内独立保护业务规则和状态流转的聚合根。
