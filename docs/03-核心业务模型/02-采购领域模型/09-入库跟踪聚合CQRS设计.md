...
在采购侧保存ASN、收货、质检、上架和库存入库事实快照，用于跟踪采购执行进度，不直接维护库存余额。
...
| 子域类型 | 核心域，采购执行进度视图 |
| 聚合根 | 入库跟踪 |
| 数据主权 | 采购上下文拥有 `入库跟踪` 的生命周期、状态、业务规则和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | 采购员、供应商系统、WMS、中央库存 |
...
| 采购完成判定服务 | 围绕入库跟踪的状态、不变量、外部事实快照和策略配置进行业务判定。 |
...
sequenceDiagram
    ...
    Aggregate->>AppService: 创建或更新到货跟踪记录
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 入库跟踪列表读模型 | 查询、分页、筛选 | 单号、状态、供应商、金额、数量、更新时间 |
| 入库跟踪详情读模型 | 详情页展示 | 单头、明细、状态历史、事件历史、操作日志 |
| 入库跟踪异常读模型 | 异常处理看板 | 异常类型、责任人、处理状态、阻塞原因 |
