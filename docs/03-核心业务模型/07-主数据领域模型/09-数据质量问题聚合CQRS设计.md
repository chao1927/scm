...
发现并治理主数据重复、缺失、过期、引用断裂、字段不规范和疑似合并问题，支撑冻结、修复、合并和关闭。
...
| --- | --- | --- | --- |
| 数据质量问题已发现 | 发现质量问题 | 聚合ID、类型编码、主数据编码、版本号、状态`待处理`、变更摘要、目标系统、操作者、幂等键 | 采购、OMS、WMS、中央库存、BMS、供应商系统、读模型、审计日志 |
...
| 类型 | 设计 |
| --- | --- |
| 异常 | 重复记录误判、引用影响范围过大、合并后历史追溯断裂、冻结影响未完业务、治理超时。 |
