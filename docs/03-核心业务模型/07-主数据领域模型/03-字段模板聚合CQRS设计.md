...
定义每类主数据有哪些字段、字段类型、必填、唯一、枚举、引用、关键字段和页面可见性，作为建档、导入、审核和发布的校验依据；物流类模板还要约束物流商能力、物流产品时效、重量体积限制、地址区域、禁运规则、面单字段和轨迹节点映射。
...
| 创建物流字段模板 | 主数据管理员 | 为物流商、物流产品、地址区域、面单模板、轨迹接口等类型配置字段，校验 TMS 必填、枚举和引用约束 | 物流字段模板校验服务 | 字段模板已创建 |
...
| tmsFieldConstraint | TMS字段约束 | 值对象 | 是 | 创建字段模板、变更字段定义 | 定义服务区域、限重限体积、禁运规则、面单映射、轨迹节点、接口鉴权字段是否必填、是否关键字段 |
...
| 创建物流字段模板 | 主数据管理员 | 加载字段模板聚合，校验 TMS 必填字段、枚举、引用类型、兼容历史记录和发布范围，执行聚合行为并写入事件发布表 | 物流字段模板校验服务 | 字段模板已创建 |
...
| 创建物流字段模板 | 物流类主数据类型已启用，TMS 必填字段、枚举和引用关系完整，命令未重复 | 修改字段模板状态为`草稿`，记录 TMS 字段约束，产生`字段模板已创建` | 删除已发布关键字段必须新增模板版本；缺少面单或轨迹关键字段则禁止发布 |
...
| 物流字段模板校验服务 | 校验物流类字段是否足以支撑 TMS 承运校验、线路匹配、面单打印、轨迹订阅和费用归因，并识别变更后是否需要重新发布到 TMS。 |

## 7. 事件产生逻辑

| 领域事件 | 触发命令 | 关键载荷 | 主要消费者 |
| --- | --- | --- | --- |
| 字段模板已创建 | 创建字段模板、创建物流字段模板 | 聚合ID、类型编码、主数据编码、版本号、状态`草稿`、变更摘要、目标系统、TMS字段约束、操作者、幂等键 | 采购、OMS、WMS、中央库存、TMS、BMS、供应商系统、读模型、审计日志 |
| 字段模板已发布 | 发布字段模板 | 聚合ID、类型编码、主数据编码、版本号、状态`已发布`、变更摘要、目标系统、TMS字段约束、操作者、幂等键 | 采购、OMS、WMS、中央库存、TMS、BMS、供应商系统、读模型、审计日志 |
| 字段模板已变更 | 变更字段定义 | 聚合ID、类型编码、主数据编码、版本号、状态`已发布`、变更摘要、目标系统、TMS字段约束、操作者、幂等键 | 采购、OMS、WMS、中央库存、TMS、BMS、供应商系统、读模型、审计日志 |
| 字段模板已停用 | 停用字段模板 | 聚合ID、类型编码、主数据编码、版本号、状态`已停用`、变更摘要、目标系统、TMS字段约束、操作者、幂等键 | 采购、OMS、WMS、中央库存、TMS、BMS、供应商系统、读模型、审计日志 |
...
| 类型 | 设计 |
| --- | --- |
| 异常 | 必填字段删除、字段类型不兼容、枚举引用失效、唯一字段重复、关键字段未配置审批策略、物流类模板缺少服务区域/限重限体积/面单映射/轨迹节点等 TMS 必要字段。 |
...
| 物流字段模板读模型 | 查看物流商、物流产品、地址区域、禁运规则、面单模板、轨迹接口配置的字段约束和发布影响 | 字段模板事件 + TMS 字段约束 | 事件投影更新 |
...
关键假设：主数据系统拥有基础资料权威口径；TMS 使用字段模板约束物流类主数据，不在本地私自扩展关键字段口径。
