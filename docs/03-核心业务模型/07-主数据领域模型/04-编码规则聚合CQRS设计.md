...
为不同主数据类型配置编码前缀、日期格式、流水长度、重置周期和号段策略，保证系统内编码唯一且可追溯；物流商、物流产品、地址区域、线路服务范围、面单模板和轨迹接口配置也必须使用主数据编码，供 TMS 稳定引用。
...
| logisticsCodeScope | 物流编码范围 | 值对象 | 是 | 创建编码规则、生成主数据编码 | 标识物流商、物流产品、地址区域、线路服务范围、面单模板、轨迹接口配置等编码适用范围 |
...
| 物流编码规则校验服务 | 校验物流类编码是否便于 TMS、OMS、WMS、BMS 共同引用，避免承运商编码、物流产品编码、地址区域编码和面单模板编码冲突。 |
...
| 主数据编码已生成 | 生成主数据编码 | 聚合ID、类型编码、主数据编码、版本号、状态`已使用`、变更摘要、目标系统、物流编码范围、操作者、幂等键 | 采购、OMS、WMS、中央库存、TMS、BMS、供应商系统、读模型、审计日志 |
...
| 类型 | 设计 |
| --- | --- |
| 异常 | 前缀冲突、流水号耗尽、重置周期错误、并发生成重复、手工录入编码与规则冲突、物流商编码或物流产品编码与 TMS 既有快照冲突。 |
...
关键假设：主数据系统拥有基础资料权威口径；TMS 引用物流类主数据编码并保存快照，不在本地生成权威物流商或物流产品编码。
