...
维护下游系统订阅范围、发布方式、主题、过滤规则、发布任务、回执、重试和补偿拉取，保证主数据最终一致分发；TMS 订阅物流商、物流产品、地址区域、禁运规则、面单模板和轨迹接口配置，并返回联调或消费回执。
...
| 主要使用角色 | 主数据管理员、系统管理员、采购、OMS、WMS、库存、TMS、BMS |
...
| 创建TMS订阅 | 主数据管理员 | 为 TMS 配置物流商、物流产品、地址区域、禁运规则、面单模板和轨迹接口配置订阅，校验主题、过滤规则和补偿拉取接口 | TMS发布路由服务 | 订阅已创建 |
...
| tmsSubscriptionRule | TMS订阅规则 | 值对象 | 是 | 创建订阅、发布主数据 | 保存 TMS 订阅主题、过滤类型、补偿拉取 API、联调回执要求和失败处理策略 |
...
| 创建TMS订阅 | 主数据管理员 | 加载发布订阅聚合，校验物流类类型、主题、过滤规则、补偿拉取 API、TMS 联调回执要求和幂等键，执行聚合行为并写入事件发布表 | TMS发布路由服务 | 订阅已创建 |
...
| 创建TMS订阅 | TMS 目标系统有效，物流类类型和版本存在，主题与过滤规则明确，补偿拉取接口可用 | 修改发布订阅状态为`已启用`，记录 TMS 订阅规则，产生`订阅已创建` | TMS 不可达或订阅过滤错误时拒绝启用；联调失败生成质量问题 |
...
| TMS发布路由服务 | 根据物流类主数据类型和 TMS 订阅规则生成主题、过滤载荷和补偿拉取策略，确保 TMS 能按版本消费物流商、产品、区域、禁运、面单和轨迹配置。 |

## 7. 事件产生逻辑

| 领域事件 | 触发命令 | 关键载荷 | 主要消费者 |
| --- | --- | --- | --- |
| 订阅已创建 | 创建订阅、创建TMS订阅 | 聚合ID、类型编码、主数据编码、版本号、状态`已启用`、变更摘要、目标系统、TMS订阅规则、操作者、幂等键 | 采购、OMS、WMS、中央库存、TMS、BMS、供应商系统、读模型、审计日志 |
| 主数据已发布 | 发布主数据 | 聚合ID、类型编码、主数据编码、版本号、状态`发布中`、变更摘要、目标系统、TMS订阅规则、操作者、幂等键 | 采购、OMS、WMS、中央库存、TMS、BMS、供应商系统、读模型、审计日志 |
| 主数据发布已确认 | 确认发布成功 | 聚合ID、类型编码、主数据编码、版本号、状态`已确认`、变更摘要、目标系统、TMS订阅规则、操作者、幂等键 | 采购、OMS、WMS、中央库存、TMS、BMS、供应商系统、读模型、审计日志 |
...
| TMS联调已通过 | 主数据发布应用服务 | 标记 TMS 订阅或物流类主数据发布确认，允许后续业务引用 | TMS+主数据编码+版本号+联调批次 |
| 承运商接口校验失败 | 主数据发布应用服务 | 标记发布失败或生成补偿任务，并通知数据质量治理 | TMS+物流商编码+接口配置版本 |
...
| 类型 | 设计 |
| --- | --- |
| 异常 | 目标系统不可达、发布载荷不兼容、订阅过滤错误、重复投递、回执超时、补偿拉取失败、TMS 订阅主题缺失、物流类主数据联调失败。 |
...
| TMS发布订阅读模型 | 查看 TMS 订阅主题、物流类主数据发布状态、联调回执和补偿拉取记录 | 发布订阅事件 + TMS 回执 | 事件投影更新 |
...
关键假设：主数据系统拥有基础资料权威口径；TMS 通过发布订阅或补偿拉取获取物流类主数据，不直接维护权威口径。
