...
> 本文用于 TMS 领域模型设计，承接 [TMS运输协同业务流程](../../02-业务流程/08-TMS运输协同业务流程.md)、[采购入库业务流程](../../02-业务流程/03-1-采购入库业务流程.md)、[销售出库业务流程](../../02-业务流程/03-2-销售出库业务流程.md)、[调拨业务流程](../../02-业务流程/06-调拨业务流程.md)、[售后退货业务流程](../../02-业务流程/07-1-售后退货业务流程.md)、[供应商退货业务流程](../../02-业务流程/07-2-供应商退货业务流程.md) 和 [核心聚合与不变量总表](../00-领域模型总览/00-核心聚合与不变量总表.md)。TMS 是运输事实源，不拥有订单、库存、仓内作业和最终账单主权。
...
| 费用来源 | TMS | 生成费用来源 | 物流费用来源 | 物流费用来源已生成 | BMS 计算运费和索赔 | 费用来源页 | 费用缺失、责任争议 |
...
| 主数据、BMS、权限 | 通用/支撑域 | 提供物流商、地址、计费和权限能力 | TMS 遵奉主数据，BMS 消费 TMS 费用来源 |
...
负责范围：运输任务、承运校验、运单创建、面单生成、发货交接接收、轨迹追加、到达通知、签收/拒收、物流异常、费用来源生成、承运商回调幂等。
...
生产事件：运输任务已创建、运单已创建、面单已生成、运输已发运、物流轨迹已追加、运输已到达、运输已签收、运输已拒收、物流异常已登记、物流费用来源已生成。
消费事件：物流商已启用、物流产品已启用、地址已变更、OMS履约单已创建、WMS出库已发货、ASN已提交、退供已出库、调拨已出库、售后退货已审核。
一致性要求：运单、轨迹、签收、异常在 TMS 内强约束；与 OMS、采购、WMS、BMS 最终一致；承运商回调必须幂等。
异常补偿：不可承运、运单创建失败、面单生成失败、重复轨迹、签收冲突、运输丢失、费用来源失败、承运商回调超时。
```
...
| TMS | BMS | 发布语言 | TMS 发布物流费用来源、异常责任、签收结果 | BMS 拥有最终费用明细和账单 |
| 权限系统 | TMS | 遵奉者 | 控制物流下单、取消、异常关闭、费用调整权限 | TMS 记录操作审计 |
...
| 事件 | 所属聚合 | 关键载荷 | 下游 |
| --- | --- | --- | --- |
| `TransportTaskCreated` | 运输任务 | 任务号、来源系统、来源单、场景、起止地址 | OMS、采购、WMS、调拨、读模型 |
| `WaybillCreated` | 运单 | 运单号、承运商、物流产品、来源单、面单状态 | OMS、WMS、采购、BMS |
| `ShippingLabelGenerated` | 面单 | 面单号、运单号、模板、打印状态 | WMS、供应商系统 |
| `TransportShipped` | 运单 | 运单号、交接时间、包裹、重量体积 | OMS、采购、调拨、BMS |
| `TrackingAppended` | 物流轨迹 | 运单号、节点、时间、地点、状态 | OMS、采购、WMS、读模型 |
| `TransportArrived` | 物流轨迹 | 运单号、到达节点、目的地 | WMS、调拨、采购 |
| `TransportSigned` | 签收记录 | 运单号、签收时间、签收人、签收结果 | OMS、采购、供应商、BMS |
| `TransportRejected` | 签收记录 | 运单号、拒收原因、责任方 | OMS、采购、供应商、BMS |
| `LogisticsExceptionRegistered` | 物流异常 | 运单号、异常类型、责任方、影响单据 | OMS、采购、WMS、BMS |
| `LogisticsFeeSourceGenerated` | 物流费用来源 | 运单号、承运商、费用项、计量、责任方、账期 | BMS |
...
| 费用来源 | 必须绑定运单、承运商、费用项和账期 | 失败重试，差异交 BMS 对账 |
