...
管理销售、调拨、退供等业务对库存的承诺锁定、释放、扣减、过期和关闭，防止超卖与重复扣减。
...
| 子域类型 | 核心域，库存承诺与释放 |
| 聚合根 | 库存预占 |
| 数据主权 | 中央库存拥有 `库存预占` 的数量口径、库存账本状态、库存流水、预占冻结调整结果和领域事件 |
| 主要使用角色 | OMS、调拨系统、采购退供、库存运营、WMS |
...
| 扣减预占 | 库存预占状态允许执行，库存维度、数量、来源单据、幂等键和权限有效 | 修改库存预占数量或状态并追加流水，产生事件 预占已扣减 | 状态不匹配则拒绝；数量不足则失败；重复事件按幂等返回原结果 |
