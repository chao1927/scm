...
将质检、盘点、风控、异常等库存从可用口径中隔离，并在条件满足后解冻恢复可用。
...
| 子域类型 | 核心域，库存可用隔离 |
| 聚合根 | 冻结单 |
| 数据主权 | 中央库存拥有 `冻结单` 的数量口径、库存账本状态、库存流水、预占冻结调整结果和领域事件 |
| 主要使用角色 | 库存运营、质控、仓库主管、盘点员 |
...
| 冻结原因策略服务 | 围绕冻结单的库存维度、数量不变量、可用口径、来源幂等和审批权限进行业务判定。 |
...
| 冻结单详情读模型 | 详情页展示 | 单头、明细、流水、事件历史、操作日志 |
