...
按货主、仓库、SKU、批次、库存状态等维度维护统一库存余额、可用数量、冻结数量、预占数量和库存流水。
...
| 子域类型 | 核心域，统一库存数量账本 |
| 聚合根 | 库存账户 |
| 数据主权 | 中央库存拥有 `库存账户` 的数量口径、库存账本状态、库存流水、预占冻结调整结果和领域事件 |
| 主要使用角色 | 库存运营、OMS、WMS、采购、调拨、BMS |
...
sequenceDiagram
    ...
    MessageBroker->>EventConsumer: 投递外部事件 WMS上架已完成
    ...
    Aggregate->>AppService: 增加库存余额并追加入库流水
    AppService->>Repository: 保存聚合和流水
