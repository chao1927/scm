...
比对中央库存、WMS库位库存、库存流水和快照，识别差异、确认原因，并推动调整闭环。
...
| 子域类型 | 核心域，库存账实核对 |
| 聚合根 | 库存对账单 |
| 数据主权 | 中央库存拥有 `库存对账单` 的数量口径、库存账本状态、库存流水、预占冻结调整结果和领域事件 |
| 主要使用角色 | 库存运营、仓库主管、财务审计、WMS |
...
| 确认差异原因 | 库存运营 | 围绕库存对账单执行确认差异原因，校验库存维度、数量口径、来源单据、幂等键和权限 | 库存对账服务 | 库存对账差异已确认 |
...
| 确认差异原因 | 库存对账单状态允许执行，库存维度、数量、来源单据、幂等键和权限有效 | 修改库存对账单数量或状态并追加流水，产生事件 库存对账差异已确认 | 状态不匹配则拒绝；数量不足则失败；重复事件按幂等返回原结果 |

## 6. 领域服务逻辑

| 领域服务 | 核心逻辑 |
| --- | --- |
| 库存对账服务 | 围绕库存对账单的库存维度、数量不变量、可用口径、来源幂等和审批权限进行业务判定。 |
| 差异归因服务 | 围绕库存对账单的库存维度、数量不变量、可用口径、来源幂等和审批权限进行业务判定。 |
...
sequenceDiagram
    ...
    MessageBroker->>EventConsumer: 投递外部事件 WMS库存快照已生成
    ...
    Aggregate->>AppService: 生成中央库存与WMS差异明细
    AppService->>Repository: 保存聚合和流水
...
| 库存对账单详情读模型 | 详情页展示 | 单头、明细、流水、事件历史、操作日志 |
