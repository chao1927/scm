...
- [退款结算聚合 CQRS 设计](11-退款结算聚合CQRS设计.md)

## DOC-NEXT-001 退款领域收口

- 退款结算是独立聚合，不是财务交接内部实体；它独立保护退款状态、支付回执应用和单笔金额规则。
- 跨退款累计由账单锁或退款额度行保护：已请求、待确认和已完成退款占用额度，失败/关闭释放额度。
- 支付超时进入 `CONFIRMATION_PENDING`，继续占用额度并先主动查单，不能按失败直接释放。
- 同一回执号全局只应用一次；回执、退款状态、累计投影和 Outbox 同事务提交。
