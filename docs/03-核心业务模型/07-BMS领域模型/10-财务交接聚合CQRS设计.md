...
## 13. 与退款结算聚合的边界（DOC-NEXT-001）

财务交接不创建、不重试、不关闭退款，也不应用支付回执；这些行为归[退款结算聚合](11-退款结算聚合CQRS设计.md)。财务交接只消费已确定的 `RefundCompleted` 或 `RefundManuallyClosed`，形成会计/资金交接明细。财务回执只能改变交接状态，不能反向覆盖退款支付事实。
