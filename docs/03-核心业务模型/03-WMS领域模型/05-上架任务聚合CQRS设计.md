...
指导上架员把合格品放到可用库位，把不合格品放到隔离、待退供或报废区域。
...
| 子域类型 | 核心域，入库落位作业 |
| 聚合根 | 上架任务 |
| 数据主权 | WMS 拥有 `上架任务` 的仓内作业状态、作业事实、任务执行过程和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | 上架员、仓库主管、质检员、中央库存 |
...
| --- | --- | --- | --- | --- |
| 生成上架任务 | 上架员 | 围绕上架任务执行生成上架任务，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 上架库位推荐服务 | 上架任务已创建 |
| 领取上架任务 | 上架员 | 围绕上架任务执行领取上架任务，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 上架库位推荐服务 | 上架任务已领取 |
| 确认上架 | 上架员 | 围绕上架任务执行确认上架，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 上架库位推荐服务 | 上架已完成 |
| 暂存不合格品 | 上架员 | 围绕上架任务执行暂存不合格品，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 上架库位推荐服务 | 不合格品已暂存 |
| 标记上架异常 | 上架员 | 围绕上架任务执行标记上架异常，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 上架库位推荐服务 | 上架异常已创建 |
...
| --- | --- | --- | --- | --- | --- |
| 上架任务Id | 上架任务ID | 聚合根 | 否 | 生成上架任务 | 全局唯一 |
| 上架任务No | 上架任务单号 | 值对象 | 否 | 生成上架任务 | 按WMS单号规则生成 |
| warehouseId | 仓库ID | 外部事实快照 | 否 | 生成上架任务 | 来源于主数据，作业过程中不可随意变更 |
...
4. 通过资源库加载 `上架任务` 聚合根，新建场景先校验业务唯一性。
...
| --- | --- | --- | --- |
| 生成上架任务 | 上架任务状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改上架任务状态或明细并产生事件 上架任务已创建 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |
| 领取上架任务 | 上架任务状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改上架任务状态或明细并产生事件 上架任务已领取 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |
| 确认上架 | 上架任务状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改上架任务状态或明细并产生事件 上架已完成 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |

## 6. 领域服务逻辑

| 领域服务 | 核心逻辑 |
| --- | --- |
| 上架库位推荐服务 | 围绕上架任务的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
| 上架数量校验服务 | 围绕上架任务的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
| 改库位判定服务 | 围绕上架任务的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |

## 7. 事件产生逻辑

| 领域事件 | 触发命令 | 关键载荷 | 主要消费者 |
| --- | --- | --- | --- |
| 上架任务已创建 | 生成上架任务 | 上架任务ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 上架任务已领取 | 领取上架任务 | 上架任务ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 上架已完成 | 确认上架 | 上架任务ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 不合格品已暂存 | 暂存不合格品 | 上架任务ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 上架异常已创建 | 标记上架异常 | 上架任务ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
...
| --- | --- | --- | --- |
| 质检已完成 | 外部事件消费服务 | 生成合格品或不合格品上架任务 | 来源上下文+事件编号+业务主键 |
...
sequenceDiagram
    ...
    User->>CommandAPI: 上架员提交命令 生成上架任务
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载上架任务聚合
...
sequenceDiagram
    ...
    User->>CommandAPI: 上架员提交命令 领取上架任务
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载上架任务聚合
    Repository->>AppService: 返回聚合当前状态
    AppService->>DomainService: 执行业务规则校验
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 上架任务已领取
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 上架任务已领取
```
...
sequenceDiagram
    ...
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 上架已完成
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 上架已完成
```
...
sequenceDiagram
    ...
    Aggregate->>AppService: 生成合格品或不合格品上架任务
...
sequenceDiagram
    ...
    AppService->>Aggregate: 执行命令 生成上架任务
    Aggregate->>EventLog: 产生事件 上架任务已创建
    EventLog->>ReadModel: 投影状态 待上架
    AppService->>Aggregate: 执行命令 领取上架任务
    Aggregate->>EventLog: 产生事件 上架任务已领取
    EventLog->>ReadModel: 投影状态 上架中
    AppService->>Aggregate: 执行命令 确认上架
    Aggregate->>EventLog: 产生事件 上架已完成
    EventLog->>ReadModel: 投影状态 已完成
    AppService->>Aggregate: 执行命令 暂存不合格品
    Aggregate->>EventLog: 产生事件 不合格品已暂存
    EventLog->>ReadModel: 投影状态 已完成
    AppService->>Aggregate: 执行命令 标记上架异常
    Aggregate->>EventLog: 产生事件 上架异常已创建
...
| 类型 | 规则 |
| --- | --- |
| 聚合不变量 | `上架任务` 的状态只能通过聚合根行为推进，内部实体不能被外部直接修改 |
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 上架任务列表读模型 | 查询、分页、筛选 | 单号、仓库、状态、作业类型、数量、更新时间 |
| 上架任务详情读模型 | 详情页和作业页展示 | 单头、明细、库位、批次、容器、状态历史、操作日志 |
| 上架任务异常读模型 | 异常看板和主管处理 | 异常类型、责任人、阻塞原因、处理状态 |
| 上架任务效率读模型 | 作业效率和绩效统计 | 操作人、开始时间、完成时间、件数、差异数 |

## 12. 设计结论与待确认问题

### 12.1 设计结论

- `上架任务` 是 WMS 领域内独立保护仓内作业规则和状态流转的聚合根。
...
## 13. 多类型上架与记账规则（DOC-NEXT-001）

- PURCHASE：合格或让步批准数量进入允许质量状态的库位。
- TRANSFER：目标仓上架完成后才发布可记账事实；送达、到仓和收货扫描不得提前增加可用。
- SALES_RETURN：仅良品进入可用；残次/冻结进入对应位置，报废/无单异常不进入可用库存。
- `PutawayCompleted` 携带 `inboundType/sourceOrderNo/qualityDisposition/putawayQty`，以来源事件号保证只记一次。
