...
处理短收、超收、错货、破损、库位不符、短拣、复核差异、发货漏扫等异常，形成责任和处理闭环。
...
| 子域类型 | 核心域，仓内异常闭环 |
| 聚合根 | 仓内异常 |
| 数据主权 | WMS 拥有 `仓内异常` 的仓内作业状态、作业事实、任务执行过程和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | 仓库主管、收货员、质检员、拣货员、复核员、发货员 |
...
| 领域服务 | 核心逻辑 |
| --- | --- |
| 仓内异常归因服务 | 围绕仓内异常的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
...
| --- | --- | --- | --- |
| 仓内异常已创建 | 创建异常 | 仓内异常ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
...
sequenceDiagram
    ...
    MessageBroker->>EventConsumer: 投递外部事件 收货差异已记录
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 仓内异常列表读模型 | 查询、分页、筛选 | 单号、仓库、状态、作业类型、数量、更新时间 |
| 仓内异常详情读模型 | 详情页和作业页展示 | 单头、明细、库位、批次、容器、状态历史、操作日志 |
| 仓内异常异常读模型 | 异常看板和主管处理 | 异常类型、责任人、阻塞原因、处理状态 |
