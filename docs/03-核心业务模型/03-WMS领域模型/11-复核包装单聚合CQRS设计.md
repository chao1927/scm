...
复核拣货结果，完成打包、称重、贴面单并生成可交接承运商的包裹。
...
| 子域类型 | 核心域，发货前质量闸口 |
| 聚合根 | 复核包装单 |
| 数据主权 | WMS 拥有 `复核包装单` 的仓内作业状态、作业事实、任务执行过程和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | 复核员、包装员、发货员、物流商系统 |
...
| 面单打印校验服务 | 围绕复核包装单的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
...
| 包装已完成 | 确认包装完成 | 复核包装单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 复核包装单列表读模型 | 查询、分页、筛选 | 单号、仓库、状态、作业类型、数量、更新时间 |
| 复核包装单详情读模型 | 详情页和作业页展示 | 单头、明细、库位、批次、容器、状态历史、操作日志 |
