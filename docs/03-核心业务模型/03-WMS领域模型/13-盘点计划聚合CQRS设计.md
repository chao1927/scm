...
组织库位或SKU范围盘点，记录账面数、实盘数、复盘结果和可审核盘点差异。
...
| 子域类型 | 核心域，账实核对 |
| 聚合根 | 盘点计划 |
| 数据主权 | WMS 拥有 `盘点计划` 的仓内作业状态、作业事实、任务执行过程和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | 仓库主管、盘点员、中央库存 |
...
| --- | --- | --- | --- | --- |
| 创建盘点计划 | 盘点员 | 围绕盘点计划执行创建盘点计划，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 盘点范围校验服务 | 盘点计划已创建 |
| 下发盘点任务 | 盘点员 | 围绕盘点计划执行下发盘点任务，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 盘点范围校验服务 | 盘点计划已下发 |
| 提交实盘 | 盘点员 | 围绕盘点计划执行提交实盘，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 盘点范围校验服务 | 盘点任务已完成 |
| 确认盘点差异 | 盘点员 | 围绕盘点计划执行确认盘点差异，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 盘点范围校验服务 | 盘点差异已创建 |
| 完成盘点计划 | 盘点员 | 围绕盘点计划执行完成盘点计划，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 盘点范围校验服务 | 盘点计划已完成 |
...
| --- | --- | --- | --- | --- | --- |
| 盘点计划Id | 盘点计划ID | 聚合根 | 否 | 创建盘点计划 | 全局唯一 |
| 盘点计划No | 盘点计划单号 | 值对象 | 否 | 创建盘点计划 | 按WMS单号规则生成 |
| warehouseId | 仓库ID | 外部事实快照 | 否 | 创建盘点计划 | 来源于主数据，作业过程中不可随意变更 |
...
4. 通过资源库加载 `盘点计划` 聚合根，新建场景先校验业务唯一性。
...
| --- | --- | --- | --- |
| 创建盘点计划 | 盘点计划状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改盘点计划状态或明细并产生事件 盘点计划已创建 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |
| 下发盘点任务 | 盘点计划状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改盘点计划状态或明细并产生事件 盘点计划已下发 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |
| 提交实盘 | 盘点计划状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改盘点计划状态或明细并产生事件 盘点任务已完成 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |

## 6. 领域服务逻辑

| 领域服务 | 核心逻辑 |
| --- | --- |
| 盘点范围校验服务 | 围绕盘点计划的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
| 盘点差异判定服务 | 围绕盘点计划的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
| 复盘审批判定服务 | 围绕盘点计划的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |

## 7. 事件产生逻辑

| 领域事件 | 触发命令 | 关键载荷 | 主要消费者 |
| --- | --- | --- | --- |
| 盘点计划已创建 | 创建盘点计划 | 盘点计划ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 盘点计划已下发 | 下发盘点任务 | 盘点计划ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 盘点任务已完成 | 提交实盘 | 盘点计划ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 盘点差异已创建 | 确认盘点差异 | 盘点计划ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 盘点计划已完成 | 完成盘点计划 | 盘点计划ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
...
| --- | --- | --- | --- |
| 库位已更新 | 外部事件消费服务 | 刷新可盘点库位范围 | 来源上下文+事件编号+业务主键 |
...
sequenceDiagram
    ...
    User->>CommandAPI: 盘点员提交命令 创建盘点计划
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载盘点计划聚合
...
sequenceDiagram
    ...
    User->>CommandAPI: 盘点员提交命令 下发盘点任务
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载盘点计划聚合
    Repository->>AppService: 返回聚合当前状态
    AppService->>DomainService: 执行业务规则校验
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 盘点计划已下发
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 盘点计划已下发
```
...
sequenceDiagram
    ...
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 盘点任务已完成
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 盘点任务已完成
```
...
sequenceDiagram
    ...
    MessageBroker->>EventConsumer: 投递外部事件 库位已更新
    ...
    Aggregate->>AppService: 刷新可盘点库位范围
...
sequenceDiagram
    ...
    AppService->>Aggregate: 执行命令 创建盘点计划
    Aggregate->>EventLog: 产生事件 盘点计划已创建
    EventLog->>ReadModel: 投影状态 草稿
    AppService->>Aggregate: 执行命令 下发盘点任务
    Aggregate->>EventLog: 产生事件 盘点计划已下发
    EventLog->>ReadModel: 投影状态 盘点中
    AppService->>Aggregate: 执行命令 提交实盘
    Aggregate->>EventLog: 产生事件 盘点任务已完成
    EventLog->>ReadModel: 投影状态 差异处理中
    AppService->>Aggregate: 执行命令 确认盘点差异
    Aggregate->>EventLog: 产生事件 盘点差异已创建
    EventLog->>ReadModel: 投影状态 差异已创建
    AppService->>Aggregate: 执行命令 完成盘点计划
    Aggregate->>EventLog: 产生事件 盘点计划已完成
...
| 类型 | 规则 |
| --- | --- |
| 聚合不变量 | `盘点计划` 的状态只能通过聚合根行为推进，内部实体不能被外部直接修改 |
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 盘点计划列表读模型 | 查询、分页、筛选 | 单号、仓库、状态、作业类型、数量、更新时间 |
| 盘点计划详情读模型 | 详情页和作业页展示 | 单头、明细、库位、批次、容器、状态历史、操作日志 |
| 盘点计划异常读模型 | 异常看板和主管处理 | 异常类型、责任人、阻塞原因、处理状态 |
| 盘点计划效率读模型 | 作业效率和绩效统计 | 操作人、开始时间、完成时间、件数、差异数 |

## 12. 设计结论与待确认问题

### 12.1 设计结论

- `盘点计划` 是 WMS 领域内独立保护仓内作业规则和状态流转的聚合根。
