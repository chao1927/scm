...
> 本文用于 WMS 领域模型设计，承接 [WMS系统产品功能设计](../../04-子系统功能设计/03-WMS系统/01-WMS系统产品功能设计.md)、[WMS系统接口设计](../../06-子系统接口设计/03-WMS系统接口设计.md)、[采购入库业务流程](../../02-业务流程/02-采购入库业务流程.md)、[销售出库业务流程](../../02-业务流程/05-销售出库业务流程.md)、[调拨业务流程](../../02-业务流程/04-调拨业务流程.md)、[售后退货业务流程](../../02-业务流程/06-售后退货业务流程.md)、[供应商退货业务流程](../../02-业务流程/01-供应商退货业务流程.md)、[中央库存领域模型](../04-中央库存领域模型/01-中央库存领域模型.md) 和 [核心聚合与不变量总表](../00-领域模型总览/01-核心聚合与不变量总表.md)。本文不只覆盖调拨，而是覆盖 WMS 从入库、收货、质检、上架、库内库存、波次、拣货、容器、复核包装、发货、退货入库、不合格品、盘点到异常处理的完整仓内执行生命周期。
...
WMS 解决的是：仓库如何把外部业务指令和 TMS 到达/运单/面单事实转成可扫描、可分派、可追踪、可复核的实物作业任务，并把真实发生的到货登记、收货、质检、上架、拣货、打包、发货交接、盘点差异等事实反馈给采购、OMS、中央库存、BMS、01-供应商系统和 TMS。
...
| 到货登记 | 收货员/TMS       | 登记到货/回传到仓     | 入库单/收货单  | 到货已登记           | 生成或开启收货单           | 到货看板  | 无 ASN、提前/延迟到货、TMS到仓未卸货 |
...
| 复核包装 | 复核/包装员/TMS    | 扫描复核、打包、称重、获取/绑定面单 | 复核包装单/包裹 | 包装已完成           | 等待发货交接             | 复核包装页 | 复核差异、重量异常、面单缺失 |
| 发货   | 发货员/承运商/TMS  | 扫包裹、交接承运商      | 发货交接/包裹   | 包裹已交接 / 出库已发货 | 通知 OMS、库存、BMS、TMS | 发货交接页 | 包裹漏扫、承运商拒收、揽收差异 |
...
| 面单 | TMS 或承运商生成的运输标签，WMS 在包装时绑定到包裹 | TMS/WMS 协作 |
| 发货交接 | WMS 将已包装包裹扫描并交给承运商的仓内作业事实，不等于客户/供应商签收 | WMS 上下文 |
...
| 物流运输/TMS | 支撑域 | 管理运输任务、运单、面单、轨迹、揽收、签收、拒收和运输异常 | WMS 只消费到仓、面单、运单和揽收相关事实，发布仓内交接事实 |
...
不负责范围：不做销售审单；不做采购审批；不维护供应商准入；不拥有中央库存统一账本；不生成财务应收应付；不决定 OMS 履约策略；不维护 TMS 运输轨迹、签收、拒收和运费主权。
核心角色：收货员、质检员、上架员、拣货员、复核包装员、发货员、承运商交接人员、盘点员、仓库主管、系统管理员、TMS事件消费者。
通用语言：入库单、收货单、质检单、上架任务、库内库存、出库单、波次、拣货单、拣货任务、容器、复核包装单、包裹、盘点计划、异常记录。
核心聚合：入库单、收货单、质检单、上架任务、库内库存、出库单、波次单、拣货单、周转容器、复核包装单、发货交接、包裹、盘点计划、异常记录。
数据主权：仓内实物作业事实、库位级执行结果、任务执行状态、容器和包裹流转、包装称重、承运商交接、盘点差异和仓内异常；TMS 拥有运输任务、运单、轨迹、揽收、签收、拒收、运输异常和运费事实。
上游上下文：采购、供应商系统、OMS、TMS、调拨/库存、主数据、权限系统、物流商主数据。
下游上下文：中央库存、采购、OMS、供应商系统、TMS、BMS、报表系统。
同步命令/接口：接收入库单、消费运输到达、开始收货、提交收货、提交质检、确认上架、接收出库单、分配库位、释放波次、确认拣货、确认包装、绑定面单、创建发货交接、扫描交接包裹、确认发货、创建盘点、提交盘点、处理异常。
生产事件：入库单已接收、到货已登记、收货已完成、质检已完成、上架已完成、不合格品已暂存、出库单已接收、出库已分配、拣货单已创建、拣货任务已完成、包装已完成、包裹已交接、出库已发货、退货验收已完成、盘点差异已创建、仓内异常已创建。
消费事件：仓库已启用、库位已更新、SKU已启用、货主已启用、承运商已启用、ASN已提交、运输已到达、运单已创建、面单已生成、物流异常已登记、出库单已创建、调拨出库已创建、调拨入库已创建、退供申请已批准、库存已预占、出库取消已请求。
一致性要求：单个作业聚合内部强一致；WMS 与中央库存、采购、OMS、BMS 之间最终一致；外部指令和事件消费必须幂等。
异常补偿：重复接单、TMS 到仓未收货、收货差异、质检不合格、上架库位满、分配失败、短拣、复核差异、面单缺失、发货漏扫、承运商拒收、盘点差异、取消冲突。
主要风险：WMS 事实与中央库存不一致；不合格品进入可用库位；拣货无库位来源；包裹未交接但 OMS 已发货；WMS 将 TMS 到仓误当收货；WMS 将承运商揽收误当客户签收；盘点差异无审批直接调账。
```
...
| TMS | WMS | 发布语言 | TMS 回传运输到达、运单创建、面单生成、物流异常，WMS 用于到货准备、包装绑定和交接校验 | TMS 是运输任务、轨迹、签收和异常权威 |
| WMS | TMS | 发布语言 | WMS 发布包装完成、包裹已交接、出库已发货、到货已登记、收货完成等仓内事实 | WMS 是包裹、交接和仓内作业事实权威 |
...
6. 仓运边界清晰：TMS 到仓不等于 WMS 收货，WMS 交接不等于 TMS 揽收，TMS 签收不等于 WMS 发货事实。
...
| --- | --- | --- | --- | --- |
| 入库单 | 入库单 | 入库行、来源单快照、到达快照 | 来源单、仓库、货主、供应商/客户快照、运单号、到仓时间 | 同来源单不能重复接单；TMS 到达只能做到货准备，已取消不能收货 |
...
| 出库单 | 出库单 | 出库行、分配记录、发运要求快照 | 来源单、优先级、物流商、承运产品、时效要求 | 未预占或未分配不能拣货；已发货不能取消 |
...
| 复核包装单 | 复核包装单 | 复核行、包裹 | 重量、体积、面单、运单、承运产品 | 复核通过才能打包；面单来自 TMS/承运商；包裹交接前可取消，交接后不可静默取消 |
| 发货交接 | 发货交接 | 交接包裹、交接批次、交接异常 | 承运商、司机/车辆、交接时间、运单 | 已交接包裹不能静默撤销；WMS 交接不代表客户/供应商签收 |
...
| 包裹 | 包裹号 | 已打包、待交接、已交接、已取消 | 承运商交接单元；运输轨迹归 TMS |
...
| 到达快照 | TMS运输任务号、运单号、承运商、预计到仓、到仓时间 | 只能由 TMS 到达事件或收货员登记刷新，不自动产生收货事实 |
| 面单快照 | 面单号、运单号、承运商、承运产品、面单URL、打印状态 | 面单来源于 TMS/承运商，WMS 只保存包装和扫描所需快照 |
...
值对象：WarehouseLocation、OwnerSnapshot、SupplierSnapshot、CustomerSnapshot、ArrivalSnapshot
主要命令：接收入库单、记录运输到达、登记到货、取消入库单、关闭入库单
主要事件：入库单已接收、到货已登记、入库单已取消、入库单已关闭
核心不变量：同来源单不能重复接单；TMS 到达不等于 WMS 收货；已开始收货后不能静默取消；入库来源类型必须明确。
...
值对象：SourceOrderReference、CarrierSnapshot、CarrierProduct、Priority
主要命令：接收出库单、记录运单/面单、分配库位、取消出库单、关闭出库单、确认发货
主要事件：出库单已接收、出库已分配、出库分配失败、出库已发货、出库单已取消
核心不变量：未接单不能分配；未分配不能拣货；出库发货必须基于包装/交接事实；已发货不能取消；分配数量不能超过库位可作业数量。
...
值对象：PackageMeasure、TrackingNo、WaybillNo、LabelSnapshot、CarrierProduct
主要命令：创建包装单、扫描容器、复核商品、登记复核差异、打包称重、绑定运单面单、打印面单、确认包装完成
主要事件：包装单已创建、复核异常已创建、包装已完成、包裹已创建
核心不变量：复核未通过不能打包；包裹必须关联出库单；需要承运发货的包裹必须绑定有效运单/面单；重量和体积异常必须提示或拦截。
...
状态机必须归属于明确实体或聚合根。WMS 中入库、收货、质检、上架、出库、波次、拣货、容器、包装、包裹、发货交接、盘点和异常都有独立生命周期，不能只用一个“作业状态”表达。

### 5.1 入库单状态机

适用实体：入库单聚合根 `WmsInboundOrder`。
...
关键约束：TMS 到仓只能推动到货准备，不能自动生成收货事实；已开始收货后不能静默取消。

### 5.2 收货单状态机

适用实体：收货单聚合根 `WmsReceiptOrder`。
...
关键约束：实收数量必须来自扫码或人工修正；短收、超收、错货、破损必须形成差异记录。

### 5.3 质检单状态机

适用实体：质检单聚合根 `WmsQcOrder`。
...
关键约束：合格数量加不合格数量不能超过待检数量；不合格必须记录原因、证据和后续去向。

### 5.4 上架任务状态机

适用实体：上架任务聚合根 `WmsPutawayTask`。
...
关键约束：上架数量不能超过来源合格或待处理数量；不合格品必须进入隔离、待退供或报废库位。

### 5.5 出库单状态机

适用实体：出库单聚合根 `WmsOutboundOrder`。
...
关键约束：未预占或未分配不能拣货；已发货不能取消；发货必须基于包裹交接事实。

### 5.6 波次单状态机

适用实体：波次单聚合根 `WmsWaveOrder`。
...
关键约束：已释放波次不能随意移除已拣订单；取消波次必须同步取消未执行拣货任务。

### 5.7 拣货任务状态机

适用实体：拣货单聚合 `WmsPickingOrder` 内部实体 `PickingTask`。
...
关键约束：拣货任务必须明确库位、SKU、批次和数量；已拣数量不能超过应拣数量。

### 5.8 周转容器状态机

适用实体：周转容器聚合根 `WmsContainer`。
...
关键约束：同一容器同一时刻只能绑定一个主作业；容器释放必须确认商品已交接或清空。

### 5.9 复核包装状态机

适用实体：复核包装单聚合根 `WmsPackOrder`。
...
关键约束：复核通过才能打包；需要承运发货的包裹必须绑定有效运单或面单。

### 5.10 包裹状态机

适用实体：复核包装单聚合内部实体 `Package`。
...
关键约束：已交接包裹不能静默撤销；WMS 包裹交接不等于 TMS 签收。

### 5.11 发货交接状态机

适用实体：发货交接聚合根 `WmsShipmentHandover`。

```mermaid
stateDiagram-v2
  [*] --> 草稿
  草稿 --> 交接中: 开始扫描包裹
  交接中 --> 交接异常: 漏扫/承运商拒收/包裹不符
  交接异常 --> 交接中: 处理后继续
  交接中 --> 已交接: 确认交接
  草稿 --> 已取消: 取消
  已交接 --> 已关闭: 关闭批次
...
关键约束：发货交接必须记录承运商、交接人、包裹、运单和时间；交接完成才可发布 WMS 发货事实。

### 5.12 盘点计划状态机

适用实体：盘点计划聚合根 `WmsCountPlan`。
...
关键约束：差异确认前不能直接调中央库存；复盘和审批结果必须可追溯。

### 5.13 异常记录状态机

适用实体：异常记录聚合根 `WmsExceptionRecord`。
...
关键约束：异常关闭必须有处理结论和责任归属；影响库存的异常必须通过中央库存调整或红冲闭环。
...
| 发货交接应用服务 | 创建交接、扫描包裹、确认交接、登记异常 | 校验包裹、承运商、运单和交接数量，保存交接事实 |
...
| TMS 到仓 | TMS 到达只能触发到货准备，不能自动收货或入库 | 到仓待收货、超时提醒、人工登记到货或关闭 |
| 面单绑定 | 需要承运发货的包裹必须有有效运单/面单 | 重新向 TMS 获取面单、人工换单、异常拦截 |
| 发货 | 发货交接必须扫描包裹并记录承运商和运单 | 漏扫补扫、撤销交接、承运商拒收、异常关闭 |
| 盘点 | 差异确认前不能直接调中央库存 | 复盘、审批、发布盘点差异事件 |
| 外部事件消费 | 外部指令和回调必须幂等 | 重复忽略、失败重试、人工补偿 |

## 9. 当前结论

WMS 不能只建模库存调拨。调拨只是 WMS 的一种出库来源和入库来源。完整 WMS 领域应围绕 `仓内实物作业`、`库位级执行`、`任务分派`、`扫码追踪`、`容器和包裹流转`、`包装面单`、`承运商交接`、`质量隔离`、`盘点差异` 和 `仓内异常闭环` 建模。WMS 与 TMS 的边界是：WMS 拥有仓内到货登记、包装、包裹和交接事实；TMS 拥有运输任务、运单、轨迹、揽收、签收、拒收、运输异常和运费事实。

## 10. 继续上下文

当前结论：本文是完整“WMS 领域模型”，覆盖入库接单、收货、质检、上架、库内库存、出库接单、库位分配、波次、拣货、容器、复核包装、发货交接、退货入库、不合格品、盘点、异常处理和操作日志。

关键假设：WMS 拥有仓内实物作业、包裹和交接事实；TMS 拥有运输任务、运单、轨迹、揽收、签收、拒收、运输异常和运费事实；中央库存拥有统一库存余额和库存流水；采购、OMS、供应商、调拨上下文拥有各自业务意图和状态；WMS 通过领域事件把实物结果反馈给它们。
