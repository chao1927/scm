...
承接采购、调拨、销售退货等入库指令，控制到货、取消、关闭和入库作业入口。
...
| 子域类型 | 核心域，入库作业入口 |
| 聚合根 | 入库单 |
| 数据主权 | WMS 拥有 `入库单` 的仓内作业状态、作业事实、任务执行过程和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | 收货员、仓库主管、采购系统、供应商系统、OMS |
...
| 到货登记判定服务 | 围绕入库单的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
...
| 到货已登记 | 登记到货 | 入库单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
...
sequenceDiagram
    ...
    MessageBroker->>EventConsumer: 投递外部事件 ASN已提交
    ...
    Aggregate->>AppService: 创建或更新入库单来源快照
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 入库单列表读模型 | 查询、分页、筛选 | 单号、仓库、状态、作业类型、数量、更新时间 |
