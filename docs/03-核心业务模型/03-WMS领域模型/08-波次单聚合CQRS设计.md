...
把多个出库单按库区、承运商、时效、拣货模式合并成波次，提高拣货效率。
...
| 子域类型 | 核心域，出库效率优化 |
| 聚合根 | 波次单 |
| 数据主权 | WMS 拥有 `波次单` 的仓内作业状态、作业事实、任务执行过程和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | 仓库主管、系统波次任务、拣货员 |
...
| --- | --- | --- | --- | --- |
| 生成波次 | 仓库主管 | 围绕波次单执行生成波次，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 波次生成服务 | 波次已创建 |
| 释放波次 | 仓库主管 | 围绕波次单执行释放波次，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 波次生成服务 | 波次已释放 |
| 生成拣货单 | 仓库主管 | 围绕波次单执行生成拣货单，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 波次生成服务 | 波次拣货单已生成 |
| 完成波次 | 仓库主管 | 围绕波次单执行完成波次，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 波次生成服务 | 波次已完成 |
| 取消波次 | 仓库主管 | 围绕波次单执行取消波次，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 波次生成服务 | 波次已取消 |
...
| --- | --- | --- | --- | --- | --- |
| 波次单Id | 波次单ID | 聚合根 | 否 | 生成波次 | 全局唯一 |
| 波次单No | 波次单单号 | 值对象 | 否 | 生成波次 | 按WMS单号规则生成 |
| warehouseId | 仓库ID | 外部事实快照 | 否 | 生成波次 | 来源于主数据，作业过程中不可随意变更 |
...
4. 通过资源库加载 `波次单` 聚合根，新建场景先校验业务唯一性。
...
| --- | --- | --- | --- |
| 生成波次 | 波次单状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改波次单状态或明细并产生事件 波次已创建 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |
| 释放波次 | 波次单状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改波次单状态或明细并产生事件 波次已释放 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |
| 生成拣货单 | 波次单状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改波次单状态或明细并产生事件 波次拣货单已生成 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |

## 6. 领域服务逻辑

| 领域服务 | 核心逻辑 |
| --- | --- |
| 波次生成服务 | 围绕波次单的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
| 波次释放校验服务 | 围绕波次单的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
| 波次取消判定服务 | 围绕波次单的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |

## 7. 事件产生逻辑

| 领域事件 | 触发命令 | 关键载荷 | 主要消费者 |
| --- | --- | --- | --- |
| 波次已创建 | 生成波次 | 波次单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 波次已释放 | 释放波次 | 波次单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 波次拣货单已生成 | 生成拣货单 | 波次单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 波次已完成 | 完成波次 | 波次单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 波次已取消 | 取消波次 | 波次单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
...
| --- | --- | --- | --- |
| 出库已分配 | 外部事件消费服务 | 加入可波次订单池 | 来源上下文+事件编号+业务主键 |
...
sequenceDiagram
    ...
    User->>CommandAPI: 仓库主管提交命令 生成波次
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载波次单聚合
...
sequenceDiagram
    ...
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 波次已释放
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 波次已释放
```
...
sequenceDiagram
    ...
    User->>CommandAPI: 仓库主管提交命令 生成拣货单
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载波次单聚合
    Repository->>AppService: 返回聚合当前状态
    AppService->>DomainService: 执行业务规则校验
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 波次拣货单已生成
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 波次拣货单已生成
```
...
sequenceDiagram
    ...
    MessageBroker->>EventConsumer: 投递外部事件 出库已分配
...
sequenceDiagram
    ...
    AppService->>Aggregate: 执行命令 生成波次
    Aggregate->>EventLog: 产生事件 波次已创建
    EventLog->>ReadModel: 投影状态 草稿
    AppService->>Aggregate: 执行命令 释放波次
    Aggregate->>EventLog: 产生事件 波次已释放
    EventLog->>ReadModel: 投影状态 已释放
    AppService->>Aggregate: 执行命令 生成拣货单
    Aggregate->>EventLog: 产生事件 波次拣货单已生成
    EventLog->>ReadModel: 投影状态 拣货中
    AppService->>Aggregate: 执行命令 完成波次
    Aggregate->>EventLog: 产生事件 波次已完成
    EventLog->>ReadModel: 投影状态 已完成
    AppService->>Aggregate: 执行命令 取消波次
    Aggregate->>EventLog: 产生事件 波次已取消
...
| 类型 | 规则 |
| --- | --- |
| 聚合不变量 | `波次单` 的状态只能通过聚合根行为推进，内部实体不能被外部直接修改 |
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 波次单列表读模型 | 查询、分页、筛选 | 单号、仓库、状态、作业类型、数量、更新时间 |
| 波次单详情读模型 | 详情页和作业页展示 | 单头、明细、库位、批次、容器、状态历史、操作日志 |
| 波次单异常读模型 | 异常看板和主管处理 | 异常类型、责任人、阻塞原因、处理状态 |
| 波次单效率读模型 | 作业效率和绩效统计 | 操作人、开始时间、完成时间、件数、差异数 |

## 12. 设计结论与待确认问题

### 12.1 设计结论

- `波次单` 是 WMS 领域内独立保护仓内作业规则和状态流转的聚合根。
