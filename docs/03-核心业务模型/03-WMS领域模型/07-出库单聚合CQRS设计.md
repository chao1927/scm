...
承接销售、调拨、退供等出库指令，并驱动库位分配、波次、拣货、复核、包装和发货。
...
| 子域类型 | 核心域，出库作业入口 |
| 聚合根 | 出库单 |
| 数据主权 | WMS 拥有 `出库单` 的仓内作业状态、作业事实、任务执行过程和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | OMS、调拨系统、采购系统、仓库主管、发货员 |
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 出库单列表读模型 | 查询、分页、筛选 | 单号、仓库、状态、作业类型、数量、更新时间 |
| 出库单详情读模型 | 详情页和作业页展示 | 单头、明细、库位、批次、容器、状态历史、操作日志 |
