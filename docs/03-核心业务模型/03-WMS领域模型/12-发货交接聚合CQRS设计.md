...
扫描包裹并与承运商交接，形成出库已发货事实，通知OMS、中央库存和BMS。
...
| 子域类型 | 核心域，出库最终事实 |
| 聚合根 | 发货交接 |
| 数据主权 | WMS 拥有 `发货交接` 的仓内作业状态、作业事实、任务执行过程和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | 发货员、承运商、OMS、中央库存 |
...
| 确认交接 | 发货员 | 围绕发货交接执行确认交接，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 发货包裹校验服务 | 出库已发货 |
...
| 确认交接 | 发货交接状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改发货交接状态或明细并产生事件 出库已发货 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |
...
| 承运商交接判定服务 | 围绕发货交接的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
...
| 包裹已扫描交接 | 扫描包裹 | 发货交接ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 出库已发货 | 确认交接 | 发货交接ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 发货交接列表读模型 | 查询、分页、筛选 | 单号、仓库、状态、作业类型、数量、更新时间 |
| 发货交接详情读模型 | 详情页和作业页展示 | 单头、明细、库位、批次、容器、状态历史、操作日志 |
| 发货交接异常读模型 | 异常看板和主管处理 | 异常类型、责任人、阻塞原因、处理状态 |
