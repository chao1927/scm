...
记录仓库实际收到的SKU、数量、批次、效期和收货差异，为质检、上架和库存入库提供事实来源。
...
| 子域类型 | 核心域，入库实收事实 |
| 聚合根 | 收货单 |
| 数据主权 | WMS 拥有 `收货单` 的仓内作业状态、作业事实、任务执行过程和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | 收货员、仓库主管、采购系统、供应商系统 |
...
| --- | --- | --- | --- | --- |
| 开始收货 | 收货员 | 围绕收货单执行开始收货，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 收货数量校验服务 | 收货已开始 |
| 扫描收货 | 收货员 | 围绕收货单执行扫描收货，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 收货数量校验服务 | 收货明细已扫描 |
| 登记收货差异 | 收货员 | 围绕收货单执行登记收货差异，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 收货数量校验服务 | 收货差异已记录 |
| 提交收货 | 收货员 | 围绕收货单执行提交收货，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 收货数量校验服务 | 收货已完成 |
| 关闭收货 | 收货员 | 围绕收货单执行关闭收货，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 收货数量校验服务 | 收货单已关闭 |
...
| --- | --- | --- | --- | --- | --- |
| 收货单Id | 收货单ID | 聚合根 | 否 | 开始收货 | 全局唯一 |
| 收货单No | 收货单单号 | 值对象 | 否 | 开始收货 | 按WMS单号规则生成 |
| warehouseId | 仓库ID | 外部事实快照 | 否 | 开始收货 | 来源于主数据，作业过程中不可随意变更 |
...
4. 通过资源库加载 `收货单` 聚合根，新建场景先校验业务唯一性。
...
| --- | --- | --- | --- |
| 开始收货 | 收货单状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改收货单状态或明细并产生事件 收货已开始 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |
| 扫描收货 | 收货单状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改收货单状态或明细并产生事件 收货明细已扫描 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |
| 登记收货差异 | 收货单状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改收货单状态或明细并产生事件 收货差异已记录 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |

## 6. 领域服务逻辑

| 领域服务 | 核心逻辑 |
| --- | --- |
| 收货数量校验服务 | 围绕收货单的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
| 收货差异判定服务 | 围绕收货单的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
| 收货关闭判定服务 | 围绕收货单的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |

## 7. 事件产生逻辑

| 领域事件 | 触发命令 | 关键载荷 | 主要消费者 |
| --- | --- | --- | --- |
| 收货已开始 | 开始收货 | 收货单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 收货明细已扫描 | 扫描收货 | 收货单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 收货差异已记录 | 登记收货差异 | 收货单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 收货已完成 | 提交收货 | 收货单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 收货单已关闭 | 关闭收货 | 收货单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
...
| --- | --- | --- | --- |
| 到货已登记 | 外部事件消费服务 | 创建收货作业单 | 来源上下文+事件编号+业务主键 |
...
sequenceDiagram
    ...
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 收货明细已扫描
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 收货明细已扫描
```
...
sequenceDiagram
    ...
    User->>CommandAPI: 收货员提交命令 登记收货差异
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载收货单聚合
    Repository->>AppService: 返回聚合当前状态
    AppService->>DomainService: 执行业务规则校验
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 收货差异已记录
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 收货差异已记录
```
...
sequenceDiagram
    ...
    MessageBroker->>EventConsumer: 投递外部事件 到货已登记
...
sequenceDiagram
    ...
    Aggregate->>EventLog: 产生事件 收货已开始
    EventLog->>ReadModel: 投影状态 收货中
    AppService->>Aggregate: 执行命令 扫描收货
    Aggregate->>EventLog: 产生事件 收货明细已扫描
    EventLog->>ReadModel: 投影状态 收货中
    AppService->>Aggregate: 执行命令 登记收货差异
    Aggregate->>EventLog: 产生事件 收货差异已记录
    EventLog->>ReadModel: 投影状态 异常
    AppService->>Aggregate: 执行命令 提交收货
    Aggregate->>EventLog: 产生事件 收货已完成
    EventLog->>ReadModel: 投影状态 已收货
    AppService->>Aggregate: 执行命令 关闭收货
    Aggregate->>EventLog: 产生事件 收货单已关闭
...
| 类型 | 规则 |
| --- | --- |
| 聚合不变量 | `收货单` 的状态只能通过聚合根行为推进，内部实体不能被外部直接修改 |
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 收货单列表读模型 | 查询、分页、筛选 | 单号、仓库、状态、作业类型、数量、更新时间 |
| 收货单详情读模型 | 详情页和作业页展示 | 单头、明细、库位、批次、容器、状态历史、操作日志 |
| 收货单异常读模型 | 异常看板和主管处理 | 异常类型、责任人、阻塞原因、处理状态 |
| 收货单效率读模型 | 作业效率和绩效统计 | 操作人、开始时间、完成时间、件数、差异数 |

## 12. 设计结论与待确认问题

### 12.1 设计结论

- `收货单` 是 WMS 领域内独立保护仓内作业规则和状态流转的聚合根。
...
## 13. 多类型收货不变量（DOC-NEXT-001）

- 通用：累计实收不得超过类型上限；到达事件不产生实收；扫描按入库单、行、条码/容器和幂等键去重。
- PURCHASE：上限来自采购可收量/ASN，超收必须有采购策略授权。
- TRANSFER：上限来自调拨已出库量，最终收货必须携带 `finalReceipt`，短收/破损发布差异事实。
- SALES_RETURN：上限来自 RMA 或人工确认；无单、错退、多退进入异常区，不自动记库存或退款。
