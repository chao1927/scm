...
对收货商品做合格、不合格、部分合格、免检判定，并决定后续上架或隔离。
...
| 子域类型 | 核心域，质量判定事实 |
| 聚合根 | 质检单 |
| 数据主权 | WMS 拥有 `质检单` 的仓内作业状态、作业事实、任务执行过程和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | 质检员、质控主管、收货员、采购系统 |
...
| --- | --- | --- | --- | --- |
| 创建质检单 | 质检员 | 围绕质检单执行创建质检单，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 质检规则判定服务 | 质检单已创建 |
| 开始质检 | 质检员 | 围绕质检单执行开始质检，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 质检规则判定服务 | 质检已开始 |
| 提交质检结果 | 质检员 | 围绕质检单执行提交质检结果，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 质检规则判定服务 | 质检已完成 |
| 冻结不合格品 | 质检员 | 围绕质检单执行冻结不合格品，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 质检规则判定服务 | 不合格品已判定 |
| 关闭质检单 | 质检员 | 围绕质检单执行关闭质检单，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 质检规则判定服务 | 质检单已关闭 |
...
| --- | --- | --- | --- | --- | --- |
| 质检单Id | 质检单ID | 聚合根 | 否 | 创建质检单 | 全局唯一 |
| 质检单No | 质检单单号 | 值对象 | 否 | 创建质检单 | 按WMS单号规则生成 |
| warehouseId | 仓库ID | 外部事实快照 | 否 | 创建质检单 | 来源于主数据，作业过程中不可随意变更 |
...
4. 通过资源库加载 `质检单` 聚合根，新建场景先校验业务唯一性。
...
| --- | --- | --- | --- |
| 创建质检单 | 质检单状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改质检单状态或明细并产生事件 质检单已创建 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |
| 开始质检 | 质检单状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改质检单状态或明细并产生事件 质检已开始 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |
| 提交质检结果 | 质检单状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改质检单状态或明细并产生事件 质检已完成 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |

## 6. 领域服务逻辑

| 领域服务 | 核心逻辑 |
| --- | --- |
| 质检规则判定服务 | 围绕质检单的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
| 不合格原因判定服务 | 围绕质检单的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
| 质检关闭判定服务 | 围绕质检单的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |

## 7. 事件产生逻辑

| 领域事件 | 触发命令 | 关键载荷 | 主要消费者 |
| --- | --- | --- | --- |
| 质检单已创建 | 创建质检单 | 质检单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 质检已开始 | 开始质检 | 质检单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 质检已完成 | 提交质检结果 | 质检单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 不合格品已判定 | 冻结不合格品 | 质检单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 质检单已关闭 | 关闭质检单 | 质检单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
...
| --- | --- | --- | --- |
| 收货已完成 | 外部事件消费服务 | 创建待检质检单 | 来源上下文+事件编号+业务主键 |
...
sequenceDiagram
    ...
    User->>CommandAPI: 质检员提交命令 创建质检单
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载质检单聚合
...
sequenceDiagram
    ...
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 质检已开始
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 质检已开始
```
...
sequenceDiagram
    ...
    User->>CommandAPI: 质检员提交命令 提交质检结果
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载质检单聚合
    Repository->>AppService: 返回聚合当前状态
    AppService->>DomainService: 执行业务规则校验
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 质检已完成
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 质检已完成
```
...
sequenceDiagram
    ...
    MessageBroker->>EventConsumer: 投递外部事件 收货已完成
...
sequenceDiagram
    ...
    AppService->>Aggregate: 执行命令 创建质检单
    Aggregate->>EventLog: 产生事件 质检单已创建
    EventLog->>ReadModel: 投影状态 待检
    AppService->>Aggregate: 执行命令 开始质检
    Aggregate->>EventLog: 产生事件 质检已开始
    EventLog->>ReadModel: 投影状态 质检中
    AppService->>Aggregate: 执行命令 提交质检结果
    Aggregate->>EventLog: 产生事件 质检已完成
    EventLog->>ReadModel: 投影状态 已完成
    AppService->>Aggregate: 执行命令 冻结不合格品
    Aggregate->>EventLog: 产生事件 不合格品已判定
    EventLog->>ReadModel: 投影状态 已完成
    AppService->>Aggregate: 执行命令 关闭质检单
    Aggregate->>EventLog: 产生事件 质检单已关闭
...
| 类型 | 规则 |
| --- | --- |
| 聚合不变量 | `质检单` 的状态只能通过聚合根行为推进，内部实体不能被外部直接修改 |
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 质检单列表读模型 | 查询、分页、筛选 | 单号、仓库、状态、作业类型、数量、更新时间 |
| 质检单详情读模型 | 详情页和作业页展示 | 单头、明细、库位、批次、容器、状态历史、操作日志 |
| 质检单异常读模型 | 异常看板和主管处理 | 异常类型、责任人、阻塞原因、处理状态 |
| 质检单效率读模型 | 作业效率和绩效统计 | 操作人、开始时间、完成时间、件数、差异数 |

## 12. 设计结论与待确认问题

### 12.1 设计结论

- `质检单` 是 WMS 领域内独立保护仓内作业规则和状态流转的聚合根。
...
## 13. 多类型质检不变量（DOC-NEXT-001）

- PURCHASE：`合格 + 不合格 + 隔离 = 已质检`；让步接收引用审批证据并通知采购/供应商。
- TRANSFER：记录短少、破损和质量处置，但差异终态归中央库存调拨聚合确认。
- SALES_RETURN：`良品 + 残次 + 冻结 + 报废 + 无单异常 = 实收`，五类互斥；修正产生新事件，不覆盖旧事实。
- 质检事件携带 `inboundType` 和来源引用，消费者不得跨类型解释处置码。
