...
维护WMS视角下仓库、库区、库位、SKU、批次、质量状态、容器上的实物库存和作业锁定。
...
| 子域类型 | 核心域，库位级实物视图 |
| 聚合根 | 库内库存 |
| 数据主权 | WMS 拥有 `库内库存` 的仓内作业状态、作业事实、任务执行过程和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | 仓库主管、上架员、拣货员、盘点员、中央库存 |
...
| --- | --- | --- | --- | --- |
| 入库上账 | 系统 | 围绕库内库存执行入库上账，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 库内数量一致性服务 | 库内库存已增加 |
| 库内移动 | 系统 | 围绕库内库存执行库内移动，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 库内数量一致性服务 | 库内库存已移动 |
| 库存冻结 | 系统 | 围绕库内库存执行库存冻结，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 库内数量一致性服务 | 库内库存已冻结 |
| 库存解冻 | 系统 | 围绕库内库存执行库存解冻，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 库内数量一致性服务 | 库内库存已解冻 |
| 盘点调整 | 系统 | 围绕库内库存执行盘点调整，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 库内数量一致性服务 | 库内库存已调整 |
...
| --- | --- | --- | --- | --- | --- |
| 库内库存Id | 库内库存ID | 聚合根 | 否 | 入库上账 | 全局唯一 |
| 库内库存No | 库内库存单号 | 值对象 | 否 | 入库上账 | 按WMS单号规则生成 |
| warehouseId | 仓库ID | 外部事实快照 | 否 | 入库上账 | 来源于主数据，作业过程中不可随意变更 |
...
4. 通过资源库加载 `库内库存` 聚合根，新建场景先校验业务唯一性。
...
| --- | --- | --- | --- |
| 入库上账 | 库内库存状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改库内库存状态或明细并产生事件 库内库存已增加 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |
| 库内移动 | 库内库存状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改库内库存状态或明细并产生事件 库内库存已移动 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |
| 库存冻结 | 库内库存状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改库内库存状态或明细并产生事件 库内库存已冻结 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |

## 6. 领域服务逻辑

| 领域服务 | 核心逻辑 |
| --- | --- |
| 库内数量一致性服务 | 围绕库内库存的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
| 库位状态校验服务 | 围绕库内库存的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
| 库存事务幂等服务 | 围绕库内库存的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |

## 7. 事件产生逻辑

| 领域事件 | 触发命令 | 关键载荷 | 主要消费者 |
| --- | --- | --- | --- |
| 库内库存已增加 | 入库上账 | 库内库存ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 库内库存已移动 | 库内移动 | 库内库存ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 库内库存已冻结 | 库存冻结 | 库内库存ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 库内库存已解冻 | 库存解冻 | 库内库存ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 库内库存已调整 | 盘点调整 | 库内库存ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
...
| --- | --- | --- | --- |
| 上架已完成 | 外部事件消费服务 | 增加库位实物数量并记录事务 | 来源上下文+事件编号+业务主键 |
...
sequenceDiagram
    ...
    User->>CommandAPI: 系统提交命令 入库上账
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载库内库存聚合
...
sequenceDiagram
    ...
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 库内库存已移动
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 库内库存已移动
```
...
sequenceDiagram
    ...
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 库内库存已冻结
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 库内库存已冻结
```
...
sequenceDiagram
    ...
    MessageBroker->>EventConsumer: 投递外部事件 上架已完成
    ...
    Aggregate->>AppService: 增加库位实物数量并记录事务
...
sequenceDiagram
    ...
    AppService->>Aggregate: 执行命令 入库上账
    Aggregate->>EventLog: 产生事件 库内库存已增加
    EventLog->>ReadModel: 投影状态 可用
    AppService->>Aggregate: 执行命令 库内移动
    Aggregate->>EventLog: 产生事件 库内库存已移动
    EventLog->>ReadModel: 投影状态 已移动
    AppService->>Aggregate: 执行命令 库存冻结
    Aggregate->>EventLog: 产生事件 库内库存已冻结
    EventLog->>ReadModel: 投影状态 冻结
    AppService->>Aggregate: 执行命令 库存解冻
    Aggregate->>EventLog: 产生事件 库内库存已解冻
    EventLog->>ReadModel: 投影状态 可用
    AppService->>Aggregate: 执行命令 盘点调整
    Aggregate->>EventLog: 产生事件 库内库存已调整
    EventLog->>ReadModel: 投影状态 已调整
```
...
| 类型 | 规则 |
| --- | --- |
| 聚合不变量 | `库内库存` 的状态只能通过聚合根行为推进，内部实体不能被外部直接修改 |
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 库内库存列表读模型 | 查询、分页、筛选 | 单号、仓库、状态、作业类型、数量、更新时间 |
| 库内库存详情读模型 | 详情页和作业页展示 | 单头、明细、库位、批次、容器、状态历史、操作日志 |
| 库内库存异常读模型 | 异常看板和主管处理 | 异常类型、责任人、阻塞原因、处理状态 |
| 库内库存效率读模型 | 作业效率和绩效统计 | 操作人、开始时间、完成时间、件数、差异数 |

## 12. 设计结论与待确认问题

### 12.1 设计结论

- `库内库存` 是 WMS 领域内独立保护仓内作业规则和状态流转的聚合根。
