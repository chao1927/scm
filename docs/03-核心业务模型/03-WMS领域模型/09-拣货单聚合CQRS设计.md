...
把出库需求拆成库位级拣货任务，指导拣货员从指定库位拿货并放入指定容器。
...
| 子域类型 | 核心域，库位级出库执行 |
| 聚合根 | 拣货单 |
| 数据主权 | WMS 拥有 `拣货单` 的仓内作业状态、作业事实、任务执行过程和领域事件；外部系统只能通过命令或事件协作 |
| 主要使用角色 | 拣货员、仓库主管、复核员、周转容器 |
...
| --- | --- | --- | --- | --- |
| 生成拣货单 | 拣货员 | 围绕拣货单执行生成拣货单，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 拣货任务生成服务 | 拣货单已创建 |
| 领取拣货 | 拣货员 | 围绕拣货单执行领取拣货，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 拣货任务生成服务 | 拣货单已领取 |
| 确认拣货任务 | 拣货员 | 围绕拣货单执行确认拣货任务，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 拣货任务生成服务 | 拣货任务已完成 |
| 登记拣货异常 | 拣货员 | 围绕拣货单执行登记拣货异常，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 拣货任务生成服务 | 拣货异常已创建 |
| 交接复核 | 拣货员 | 围绕拣货单执行交接复核，校验状态、来源、仓库、库位、SKU、批次和作业权限 | 拣货任务生成服务 | 拣货单已交接复核 |
...
| --- | --- | --- | --- | --- | --- |
| 拣货单Id | 拣货单ID | 聚合根 | 否 | 生成拣货单 | 全局唯一 |
| 拣货单No | 拣货单单号 | 值对象 | 否 | 生成拣货单 | 按WMS单号规则生成 |
| warehouseId | 仓库ID | 外部事实快照 | 否 | 生成拣货单 | 来源于主数据，作业过程中不可随意变更 |
...
4. 通过资源库加载 `拣货单` 聚合根，新建场景先校验业务唯一性。
...
| --- | --- | --- | --- |
| 生成拣货单 | 拣货单状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改拣货单状态或明细并产生事件 拣货单已创建 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |
| 领取拣货 | 拣货单状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改拣货单状态或明细并产生事件 拣货单已领取 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |
| 确认拣货任务 | 拣货单状态允许执行，仓库、库位、SKU、批次、数量和权限有效 | 修改拣货单状态或明细并产生事件 拣货任务已完成 | 状态不匹配则拒绝；作业差异进入仓内异常或人工待办 |

## 6. 领域服务逻辑

| 领域服务 | 核心逻辑 |
| --- | --- |
| 拣货任务生成服务 | 围绕拣货单的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
| 拣货数量校验服务 | 围绕拣货单的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |
| 拣货异常判定服务 | 围绕拣货单的作业状态、数量不变量、库位规则、批次质量状态和外部事实快照进行业务判定。 |

## 7. 事件产生逻辑

| 领域事件 | 触发命令 | 关键载荷 | 主要消费者 |
| --- | --- | --- | --- |
| 拣货单已创建 | 生成拣货单 | 拣货单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 拣货单已领取 | 领取拣货 | 拣货单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 拣货任务已完成 | 确认拣货任务 | 拣货单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 拣货异常已创建 | 登记拣货异常 | 拣货单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
| 拣货单已交接复核 | 交接复核 | 拣货单ID、仓库、SKU、批次、库位、数量、状态 | 中央库存、采购、OMS、BMS、读模型、审计日志 |
...
| --- | --- | --- | --- |
| 波次已释放 | 外部事件消费服务 | 创建拣货单和库位级拣货任务 | 来源上下文+事件编号+业务主键 |
...
sequenceDiagram
    ...
    User->>CommandAPI: 拣货员提交命令 生成拣货单
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载拣货单聚合
...
sequenceDiagram
    ...
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 拣货单已领取
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 拣货单已领取
```
...
sequenceDiagram
    ...
    User->>CommandAPI: 拣货员提交命令 确认拣货任务
    CommandAPI->>AppService: 转换命令对象
    AppService->>Repository: 加载拣货单聚合
    Repository->>AppService: 返回聚合当前状态
    AppService->>DomainService: 执行业务规则校验
    DomainService->>AppService: 返回规则校验结果
    AppService->>Aggregate: 调用聚合行为
    Aggregate->>AppService: 返回领域事件 拣货任务已完成
    AppService->>Repository: 保存聚合状态
    AppService->>Outbox: 写入事件发布表
    AppService->>AuditLog: 写入操作审计
    Outbox->>MessageBroker: 发布领域事件 拣货任务已完成
```
...
sequenceDiagram
    ...
    MessageBroker->>EventConsumer: 投递外部事件 波次已释放
    ...
    Aggregate->>AppService: 创建拣货单和库位级拣货任务
...
sequenceDiagram
    ...
    AppService->>Aggregate: 执行命令 生成拣货单
    Aggregate->>EventLog: 产生事件 拣货单已创建
    EventLog->>ReadModel: 投影状态 待拣货
    AppService->>Aggregate: 执行命令 领取拣货
    Aggregate->>EventLog: 产生事件 拣货单已领取
    EventLog->>ReadModel: 投影状态 拣货中
    AppService->>Aggregate: 执行命令 确认拣货任务
    Aggregate->>EventLog: 产生事件 拣货任务已完成
    EventLog->>ReadModel: 投影状态 拣货中
    AppService->>Aggregate: 执行命令 登记拣货异常
    Aggregate->>EventLog: 产生事件 拣货异常已创建
    EventLog->>ReadModel: 投影状态 异常
    AppService->>Aggregate: 执行命令 交接复核
    Aggregate->>EventLog: 产生事件 拣货单已交接复核
    EventLog->>ReadModel: 投影状态 已交接
```
...
| 类型 | 规则 |
| --- | --- |
| 聚合不变量 | `拣货单` 的状态只能通过聚合根行为推进，内部实体不能被外部直接修改 |
...
| 读模型 | 使用场景 | 主要字段 |
| --- | --- | --- |
| 拣货单列表读模型 | 查询、分页、筛选 | 单号、仓库、状态、作业类型、数量、更新时间 |
| 拣货单详情读模型 | 详情页和作业页展示 | 单头、明细、库位、批次、容器、状态历史、操作日志 |
| 拣货单异常读模型 | 异常看板和主管处理 | 异常类型、责任人、阻塞原因、处理状态 |
| 拣货单效率读模型 | 作业效率和绩效统计 | 操作人、开始时间、完成时间、件数、差异数 |

## 12. 设计结论与待确认问题

### 12.1 设计结论

- `拣货单` 是 WMS 领域内独立保护仓内作业规则和状态流转的聚合根。
