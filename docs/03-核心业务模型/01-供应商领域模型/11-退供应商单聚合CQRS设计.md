...
管理因质检不合格、库存异常、采购退货等产生的退供应商业务，从创建、审核、库存锁定、供应商确认、退供出库、TMS运输、供应商签收/拒收、差异、对账到关闭。
...
| 聚合根 | 退供应商单 |
| 数据主权 | 本上下文拥有 `退供应商单` 的生命周期、状态、业务规则和领域事件；TMS拥有运输任务、运单、轨迹、签收/拒收和物流异常事实；外部系统只能通过命令或事件协作，不能直接修改聚合数据 |
| 主要使用角色 | 采购员、质控人员、仓库人员、物流专员、供应商业务员、财务结算人员、系统库存/WMS/TMS/BMS事件 |
...
| 供应商确认退货 | 供应商业务员 | 确认退货责任、数量和收货地址 | 无差异进入待出库；有差异生成采购待办 | 供应商已确认退货/供应商退供差异已反馈 |
| 创建退供出库 | 退供应商单应用服务 | 发起仓库退供拣货、复核和出库 | 待出库状态下请求WMS创建退供出库单 | 退供出库已请求 |
| 记录退供出库结果 | WMS事件消费者 | 记录实际退供出库事实 | 写入出库数量、批次、包裹信息，状态进入已出库 | 退供已出库 |
| 记录退供运单 | TMS事件消费者 | 记录承运商和运单，跟踪退供运输 | 已出库进入在途，写入运单、承运商和预计到达 | 退供运输信息已记录 |
| 记录供应商签收结果 | TMS事件消费者 | 记录供应商签收事实 | 在途进入已签收或供应商差异，写入签收数量和证明 | 供应商已签收 |
| 记录供应商拒收/运输异常 | TMS事件消费者 | 处理拒收、破损、丢失、延误等运输问题 | 状态进入供应商差异或异常处理，阻塞正常关闭 | 供应商退供运输异常已记录 |
| 完成退供对账 | BMS事件消费者 | 完成应付冲减、索赔或差异结算 | 记录结算影响，满足关闭条件 | 退供对账已完成 |
| 关闭退供单 | 采购员/系统 | 关闭已完成或异常终止的退供流程 | 签收/争议结论和结算完成后关闭 | 退供应商单已关闭 |
...
| returnStatus | 退供状态 | 值对象 | 是 | 审核/锁定/确认/出库/运单/签收/关闭 | 草稿、待审核、已审核、库存锁定中、待供应商确认、待出库、已出库、在途、已签收、供应商差异、已关闭、异常关闭 |
...
| transportSnapshot | 退供运输快照 | 内部实体 | 是 | TMS事件消费 | TMS运输任务号、运单号、承运商、运输状态、最新轨迹、签收/拒收结果、物流异常 |
...
| 记录退供运单 | TMS事件消费者 | 记录运单、承运商和在途状态，已出库->在途 | 退供差异判定服务 | 退供运输信息已记录 |
| 记录供应商签收结果 | TMS事件消费者 | 在途->已签收，记录签收数量、签收时间和证明 | 退供差异判定服务 | 供应商已签收 |
| 记录供应商拒收/运输异常 | TMS事件消费者 | 记录拒收、破损、丢失、延误和责任方，进入差异或异常处理 | 退供差异判定服务 | 供应商退供运输异常已记录 |
...
| 记录退供运单 | TMS回传运单创建事实；状态已出库；出库包裹可匹配 | 写入TMS运输任务号、运单号、承运商、预计到达；状态改在途 | 运单重复按幂等处理；运单与退供单不匹配进入异常事实池 |
| 记录供应商签收结果 | TMS回传签收事实；状态在途；签收对象为目标供应商 | 写入签收数量、签收时间、签收证明；无差异进入已签收，有差异进入供应商差异 | 签收数量大于出库数量拒绝；签收证明缺失时要求补充 |
| 关闭退供单 | TMS签收或争议结论完成；结算冲减或索赔完成；差异已处理 | 状态改已关闭；释放流程待办；发布关闭事件 | 结算未完成或物流异常责任未确认时不能正常关闭，只能异常关闭并保留原因 |
...
| 退供差异判定服务 | 识别申请数量、锁定数量、出库数量、TMS签收数量、拒收数量之间的差异，并判断是否需采购、仓库、物流或供应商处理。 |
| 退供应商结算影响判定服务 | 根据退供原因、出库数量、TMS签收/拒收结果、物流异常责任、索赔规则判断应付冲减、索赔金额和关闭条件。 |
...
| 退供已出库 | 记录退供出库结果 | returnSupplierId、实际出库数量、出库单号、包裹信息 | TMS、BMS、评分、质量问题 |
| 退供运输信息已记录 | 记录退供运单 | returnSupplierId、TMS运输任务号、运单号、承运商、预计到达 | 退供读模型、评分 |
| 供应商已签收 | 记录供应商签收结果 | returnSupplierId、运单号、签收数量、签收时间、差异 | BMS、退供读模型 |
| 供应商退供运输异常已记录 | 记录供应商拒收/运输异常 | returnSupplierId、运单号、异常类型、责任方、影响数量 | 采购员待办、BMS、评分 |
...
| 运单已创建 | TMS事件消费服务 | 记录退供运单、承运商、预计到达，状态进入在途 | TMS上下文+事件编号+waybillNo |
| 运输已签收 | TMS事件消费服务 | 记录供应商签收数量、签收时间和证明，状态进入已签收或供应商差异 | TMS上下文+事件编号+waybillNo |
| 运输已拒收 | TMS事件消费服务 | 记录拒收原因和责任方，状态进入供应商差异，阻塞正常关闭 | TMS上下文+事件编号+waybillNo |
| 物流异常已登记 | TMS事件消费服务 | 记录运输异常，阻塞签收和关闭，必要时生成索赔依据 | TMS上下文+事件编号+exceptionNo |
...
sequenceDiagram
    ...
    Aggregate->>AppService: 更新签收数量和退供状态
...
| 补偿 | 事件发布失败走发布表重试；消费失败走收件箱重试；TMS运单创建失败、轨迹延迟、拒收、破损、丢失等跨上下文部分成功场景，通过补偿命令或人工待办处理 |
...
| 退供执行跟踪读模型 | 查看锁定、供应商确认、出库、运输、签收进度 | 库存锁定、确认记录、出库单、TMS运单、承运商、最新轨迹、签收结果 |
| 退供差异处理读模型 | 处理供应商确认差异、出库差异、签收/拒收/物流异常差异 | 差异类型、责任方、数量、金额、运单号、处理状态 |
