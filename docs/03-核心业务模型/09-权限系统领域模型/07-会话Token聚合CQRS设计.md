...
维护登录会话、访问令牌、刷新令牌、设备、过期时间、权限版本、失效和强制下线。TMS 后台接口、承运商回调管理页面、运单/面单/轨迹/签收/费用来源等敏感操作都必须经过 Token 校验和应用身份校验。
...
| 校验TMS敏感请求 | TMS用户 / 网关 | 校验Token、应用编码、权限点、物流商/运输场景数据范围和权限版本 | Token签发校验服务 | Token校验已通过 |
...
| applicationCode | 应用编码 | 值对象 | 是 | 签发Token / 校验Token | 标识发起请求的应用，TMS后台、承运商回调管理、费用来源推送客户端必须明确区分 |
...
| 校验TMS敏感请求 | TMS用户 / 网关 | 校验Token有效、应用为授权TMS客户端、请求动作具备权限点、物流商/运输场景数据范围未越权 | Token签发校验服务 | Token校验已通过 |
