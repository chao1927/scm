...
承接业务系统审批请求，维护审批实例、审批节点、审批任务、加签、转交、撤回、取消和审批结果。TMS 中手工换承运商、取消运单、补录轨迹、签收冲正、物流异常关闭、费用来源修正会影响履约事实和结算依据，应根据金额、时效、异常等级或数据范围触发审批。
...
| 发起TMS敏感审批 | TMS应用服务 | 围绕运单、轨迹、签收、异常、费用来源敏感操作发起审批，校验权限点、数据范围和风险策略 | 审批路由服务 | 审批已开始 |
...
| logisticsApprovalRef | 物流审批引用 | 值对象 | 是 | 发起审批 / 完成审批 | 记录TMS业务对象、敏感操作类型、风险等级和审批结果，如换承运商、取消运单、轨迹补录、签收冲正、异常关闭、费用来源修正 |
...
| 发起TMS敏感审批 | TMS应用服务 | 加载审批实例聚合，校验TMS基础权限、业务对象存在、命中审批策略后创建审批任务 | 审批路由服务 | 审批已开始 |
