...
TMS 的事件生产与消费需要覆盖采购入库、销售出库、调拨、售后退货、供应商退货、BMS 费用结算和权限审计这些主流程。

| 来源流程 | 产品能力 | 页面/操作落点 | 状态或事件影响 |
| --- | --- | --- | --- |
| 采购入库 | 支持 ASN、自送、承运商配送、到仓提醒和采购物流异常 | 运输任务、运单、轨迹、异常页按采购入库场景筛选 | 消费`ASN已提交`，生产`运输已到达/物流异常已登记/物流费用来源已生成` |
| 销售出库 | 支持 OMS 请求配送、WMS包装打单、发货交接、签收/拒收回传 | 运单管理、面单管理、签收管理增加销售履约来源单 | 消费`销售配送已请求`、`WMS包装已完成`，生产`运单已创建/运输已签收` |
| 调拨 | 支持调拨出库后在途、到达、签收、异常和运输费用 | 运输任务、轨迹、签收、费用来源按调拨单号关联 | 生产调拨运输事实给调拨系统、WMS、中央库存、BMS |
| 售后退货 | 支持上门取件、客户自寄、退货到仓、拒收和异常 | 运输任务和签收页增加售后单、退货方式、取件状态 | 消费`售后退货已审核`，生产退货运输事实 |
| 供应商退货 | 支持退供发运、供应商签收/拒收、索赔、退供运费 | 运单、签收、异常、费用来源增加退供应商来源单 | 消费`退供已出库`，生产`运输已签收/拒收` |
| BMS费用结算 | 支持费用来源生成、推送、失败重试、BMS采集回写和修正 | 物流费用来源页增加采集状态、差异、修正审批、重推 | 生产`物流费用来源已推送`，消费`BMS费用来源已采集/对账差异已发生` |
| 权限审计 | 高危物流动作必须校验权限、审批、原因和操作日志 | 运单作废、面单作废、轨迹补录、签收冲正、异常关闭、费用修正 | 消费审批结果，生产操作审计和领域事件 |
...
| 来源 | 使用方式 |
| --- | --- |
| `docs/03-核心业务模型/06-TMS领域模型` | 决定运输任务、运单、面单、轨迹、签收、异常、费用来源的状态和事件 |
| `docs/05-子系统数据库设计` | 已补充运输任务、运单、包裹、轨迹、签收、异常、费用来源、BMS采集回写、接口日志、事件表和操作审计表 |
...
下一步：基于 TMS 产品功能、数据库、接口和事件设计生成前端原型页面，或继续细化承运商下单、轨迹回调、签收冲正、费用来源修正的页面交互和权限。
