...
> 本文根据 [WMS领域模型](../03-核心业务模型/03-WMS领域模型/01-WMS领域模型.md)、[WMS系统产品功能设计](../04-子系统功能设计/WMS系统/WMS系统产品功能设计.md)、[WMS系统数据库设计](../05-子系统数据库设计/03-WMS系统数据库设计.md) 和 [上下文映射与领域事件目录](./50-上下文映射与领域事件目录.md) 设计。接口按 DDD + CQRS 口径拆分：查询接口读取 WMS 作业读模型，命令接口触发应用服务和仓内作业聚合，跨系统接口遵守命令/事件边界。
...
| 请求头 | 必填 | 适用接口 | 说明 |
| --- | --- | --- | --- |
| `Authorization` | 是 | 前端接口、PDA 接口 | `Bearer access_token`，由权限系统签发 |
...
成功事件：`ReceiptStarted`、`ReceiptLineScanned`、`ReceiptDifferenceRecorded`、`InboundOrderReceived`。
...
成功事件：`PutawayTaskClaimed`、`PutawayLocationChanged`、`InboundOrderPutawayCompleted`、`RejectedStockStored`。
...
成功事件：`OutboundOrderReviewed`、`ReviewDifferenceRecorded`、`PackageCreated`、`PackageWeighed`、`PackageCompleted`。
...
成功事件：`PackageHandoverScanned`、`OutboundOrderShipped`、`ShipmentHandoverCompleted`。
...
WMS 不直接修改中央库存余额。发货、上架、盘点差异确认后，优先发布事件；若需要同步强校验，可调用中央库存开放接口。
...
| 包装已完成 | `PackageCompleted` | 完成包装 | OMS、BMS | `outboundOrderNo`、`packages` |
| 出库单已发货 | `OutboundOrderShipped` | 确认发货 | OMS、中央库存、BMS、物流系统 | `outboundOrderNo`、`shipmentNo`、`packages`、`shippedLines` |
...
| 项 | 设计 |
| --- | --- |
| 认证 | 前端和 PDA 接口必须携带 `Authorization`，由网关或 WMS 后端调用权限系统校验 |
...
| 库存补偿 | WMS 已发货但库存扣减失败时，事件重试、人工补账、禁止重复扣减；库存以事件幂等键去重 |

## 11. 当前结论与待决问题

当前结论：WMS 接口的核心不是普通单据 CRUD，而是仓内作业命令。每个写接口都应明确作业角色、作业位置、数量、批次、容器、设备、状态推进、事件发布和幂等键；WMS 通过事件把真实发生的收货、质检、上架、拣货、复核、发货、盘点事实传给采购、OMS、中央库存、BMS 和供应商系统。
...
| 问题 | 当前建议 |
| --- | --- |
| 采购 ASN 是供应商直连 WMS 还是采购转发 | 第一版支持两种入口，但推荐由采购或集成服务统一转发，减少供应商系统直接依赖 WMS |
