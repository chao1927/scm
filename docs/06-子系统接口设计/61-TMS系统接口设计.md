...
> 本文根据 [TMS领域模型](../03-核心业务模型/09-TMS领域模型/01-TMS领域模型.md)、[TMS系统产品功能设计](../04-子系统功能设计/TMS系统/TMS系统产品功能设计.md)、[TMS系统数据库设计](../05-子系统数据库设计/09-TMS系统数据库设计.md) 和 [上下文映射与领域事件目录](./50-上下文映射与领域事件目录.md) 设计。接口按 DDD + CQRS 口径拆分：查询接口读取运输读模型，命令接口触发应用服务和运输聚合行为，跨系统接口遵守“TMS 拥有运输事实，其它系统拥有各自业务状态”的边界。
...
| 请求头 | 必填 | 适用接口 | 说明 |
| --- | --- | --- | --- |
| `Authorization` | 是 | 前端接口、内部接口 | `Bearer access_token`，由权限系统签发 |
...
下一步：补充 TMS 事件生产与消费设计，或基于本文生成 TMS 前端原型页面。
