...
| QueryTrackingEvent | OMS/采购/WMS/BMS | TMS | TrackingEvent | tracking_no + query_request_id | 轨迹列表 |
...
| 事件 | 英文代码 | 来源聚合 | 消费方 | 业务含义 |
| --- | --- | --- | --- | --- |
| 运单已创建 | WaybillCreated | Waybill | OMS、WMS、采购、BMS | TMS 已创建运输任务和运单，可用于打单、交接和费用预估 |
| 运输已发运 | TransportShipped | Waybill | OMS、采购、调拨、供应商、中央库存、BMS | 货物已交给承运商或从发货点发出 |
| 运输在途 | TrackingAppended | TrackingEvent | OMS、采购、调拨、供应商、WMS、中央库存 | 承运商轨迹显示货物在运输途中 |
| 运输已到达 | TransportArrived | TrackingEvent | WMS、采购、调拨、OMS、中央库存 | 货物到达收货仓、客户、供应商或调入仓 |
| 运输已签收 | TransportSigned | DeliveryReceipt | OMS、采购、供应商、中央库存、BMS | 收货方已签收，可推进订单完成、退供闭环或物流费用结算 |
| 物流异常已发生 | LogisticsExceptionRegistered | LogisticsException | OMS、采购、WMS、中央库存、BMS | 发生延误、破损、丢失、拒收、地址异常等问题 |
| 物流费用来源已生成 | LogisticsFeeSourceGenerated | LogisticsFeeSource | BMS | TMS 生成物流计费来源事实，供 BMS 匹配计费规则 |
...
| 55  | [01-供应商系统接口设计](01-供应商系统接口设计.md)   | 细化供应商门户页面接口、供应商协同命令接口、供应商事件发布/消费契约         |
| 56  | [02-采购系统接口设计](02-采购系统接口设计.md)     | 细化采购前端页面接口、采购跨系统命令接口和采购事件消费/发布契约           |
| 57  | [03-WMS系统接口设计](03-WMS系统接口设计.md)   | 细化 WMS 前端/PDA 作业接口、入库出库跨系统命令、WMS 事件发布/消费契约 |
| 58  | [04-中央库存系统接口设计](04-中央库存系统接口设计.md) | 细化中央库存前端页面接口、库存预占/释放/入库/扣减命令接口、库存事件发布/消费契约 |
| 59  | [05-OMS系统接口设计](05-OMS系统接口设计.md)   | 细化 OMS 前端页面接口、订单接入/审单/履约/出库/取消/售后命令接口、OMS 事件发布/消费契约 |
| 60  | [06-TMS系统接口设计](06-TMS系统接口设计.md)   | 细化物流下单、取消、面单、轨迹、签收、异常、物流费用来源事件和承运商回调接口 |
| 61  | [07-BMS系统接口设计](07-BMS系统接口设计.md)   | 细化 BMS 前端页面接口、费用采集/对账/账单/发票/财务交接命令接口、BMS 事件发布/消费契约 |
...
下一步：建议补充 `61-06-TMS系统接口设计` 和 `TMS领域模型`，因为采购收货、销售发货、售后退货、供应商退货、调拨和物流费用都依赖运输事实。
