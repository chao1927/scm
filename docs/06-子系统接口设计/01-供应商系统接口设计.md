...
| 事件消费通道 | RocketMQ；`supplier-business-event-v1` 等消费组订阅上游领域 Topic |
...
```text
RocketMQ Topics: master-data-domain-event、iam-approval-domain-event、各上游 *-domain-event
Consumer Groups: supplier-master-data-snapshot、supplier-contract-approval、supplier-business-event-v1
```
