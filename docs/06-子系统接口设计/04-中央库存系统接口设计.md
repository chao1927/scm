...
`POST/GET /api/inventory/v1/transfers`、`GET /api/inventory/v1/transfers/{transferNo}`、`POST /{transferNo}/submit|approve|reserve|cancel`、`POST /{transferNo}/difference/confirm`、`POST /internal/inventory/v1/transfer-events` 统一映射[调拨单聚合](../03-核心业务模型/04-中央库存领域模型/08-调拨单聚合CQRS设计.md)。
