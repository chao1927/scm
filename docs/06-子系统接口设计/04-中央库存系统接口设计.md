...
| 事件消费通道 | RocketMQ；`inventory-domain-event-consumer` 订阅上游领域 Topic |
...
| 消费组 | `inventory-domain-event-consumer` |
...
## 12. 调拨接口契约收口（DOC-NEXT-001）

`POST/GET /api/inventory/v1/transfers`、`GET /api/inventory/v1/transfers/{transferNo}`、`POST /{transferNo}/submit|approve|reserve|cancel`、`POST /{transferNo}/difference/confirm`，以及由 `inventory-domain-event-consumer` 消费的调拨事件，统一映射[调拨单聚合](../03-核心业务模型/04-中央库存领域模型/08-调拨单聚合CQRS设计.md)。

| 契约项 | 强制规则 |
| --- | --- |
| 写权限 | 创建/提交校验货主和源仓；审批、预占、取消、差异确认同时校验源仓与目标仓 |
| 创建幂等 | `Idempotency-Key`；同键不同业务参数返回冲突 |
| 事件幂等 | `sourceSystem + eventId`；聚合版本只用于本地乐观锁，不接受生产方版本覆盖 |
| 收货载荷 | `transferNo/eventId/receivedQty/cumulativeReceivedQty/finalReceipt/qualityDisposition/occurredAt` |
| 差异确认 | 仅 DIFFERENCE；必须提交差异类型、数量、责任方、原因、凭证引用和版本 |
| 乱序补偿 | 缺少出库/在途前置事实时进入待重放，不跨级推进，不重复记账 |

数量输出必须同时返回申请、预占、出库、在途待处理、入库和已确认差异；TMS 送达只更新轨迹，不得调用入库记账。
