...
> 本文根据 [采购领域模型](../03-核心业务模型/02-采购领域模型/01-采购系统领域模型.md)、[02-采购系统产品功能设计](../04-子系统功能设计/02-采购系统/01-采购系统产品功能设计.md)、[02-采购系统数据库设计](../05-子系统数据库设计/02-采购系统数据库设计.md) 和 [上下文映射与领域事件目录](./00-上下文映射与领域事件目录.md) 设计。接口按 DDD + CQRS 口径拆分：查询接口读取读模型，命令接口触发应用服务和聚合行为，跨系统接口遵守命令/事件边界。

## 1. 设计范围

| 类型 | 范围 | 说明 |
| --- | --- | --- |
| 前端页面接口 | 采购工作台、请购、询价、报价、比价、采购订单、供应商确认、到货跟踪、退供、采购价格、参数、枚举、操作日志 | 面向02-采购系统 Web 前端 |
| 跨系统命令接口 | 采购 -> WMS、采购 -> BMS、采购 -> 权限、采购 -> 主数据、采购 -> 供应商 | 同步请求对方系统执行动作或查询读模型 |
| 跨系统事件接口 | 供应商 -> 采购、WMS -> 采购、主数据 -> 采购、采购 -> 供应商/WMS/BMS | 异步传递已经发生的业务事实 |
| 不包含 | 库存系统内部扣减、WMS 仓内作业细节、BMS 计费明细生成规则 | 只定义02-采购系统需要调用或消费的契约 |
...
| 数据主权 | 02-采购系统拥有采购单据生命周期；供应商协同事实、仓内作业事实、主数据事实由对应上下文拥有 |
...
| 事件消费通道 | RocketMQ；`purchase-business-event-consumer` 订阅上游领域 Topic |
...
02-采购系统各单据明细统一使用以下基础字段，具体接口可按业务增减。
...
成功事件：`采购订单已提交`、`采购订单已批准`、`采购订单已发布`、`采购订单已关闭`。发布成功后向01-供应商系统发布或调用采购订单协同接口。
...
调用时机：采购订单发布且供应商确认后，02-采购系统根据 PO 或 ASN 创建/更新 WMS 入库计划；也可在供应商 ASN 到达后创建。
...
调用时机：用户进入02-采购系统、刷新页面、提交写操作前由网关或后端权限拦截器调用。
...
```text
RocketMQ Topics: supplier-domain-event、wms-domain-event、tms-domain-event、bms-domain-event
Consumer Group: purchase-business-event-consumer
```

调用方式：RocketMQ Consumer 接收上游事件，采购先写 Inbox/事件消费日志，再由对应应用服务更新本地聚合或读模型；不提供 HTTP 事件投递入口。
...
| 项 | 设计 |
| --- | --- |
| 认证 | 前端接口必须携带 `Authorization`，由网关或采购后端调用09-权限系统校验 |
...
关键假设：第一版02-采购系统采用同步命令 + 异步事件组合；事件不做完整事件溯源，但保留领域事件表、事件消费日志和操作日志。

待决问题：

| 问题                      | 当前建议                                  |
| ----------------------- | ------------------------------------- |
| 供应商报价由02-采购系统录入还是01-供应商系统提交为主 | 两者都支持：采购后台可代录，01-供应商系统提交事件作为主协同入口        |
