...
> 本文根据 [OMS领域模型](../03-核心业务模型/05-OMS领域模型/01-OMS领域模型.md)、[05-OMS系统产品功能设计](../04-子系统功能设计/05-OMS系统/01-OMS系统产品功能设计.md)、[05-OMS系统数据库设计](../05-子系统数据库设计/05-OMS系统数据库设计.md) 和 [上下文映射与领域事件目录](./00-上下文映射与领域事件目录.md) 设计。接口按 DDD + CQRS 口径拆分：查询接口读取 OMS 订单履约读模型，命令接口触发应用服务和聚合行为，跨系统接口遵守“OMS 编排履约，库存/WMS/BMS 各自拥有业务事实”的边界。
...
| TMS | 运输已签收 | `TransportSigned` | TMS事件消费服务 | 销售订单已签收，满足完成条件后订单完成 |
