...
> 当前计划入口见 [00-计划导航.md](00-计划导航.md)。模块完成度以 [05-九子系统模块状态矩阵.md](05-九子系统模块状态矩阵.md) 为准，并行领取任务以 [06-多Agent并行执行计划.md](06-多Agent并行执行计划.md) 和 [`tasks/todo.md`](../../tasks/todo.md) 为准。
...
| 项目 | 约定 |
| --- | --- |
| 后端 | JDK 17、Spring Boot、Dubbo、MyBatis、RocketMQ、MySQL、Redis |
...
| 文档 | 状态 | 用途 |
| --- | --- | --- |
| [九子系统模块状态矩阵](05-九子系统模块状态矩阵.md) | 当前 | 按子系统、模块和正式需求区分已完成、部分完成、待开发、阻塞和已迁移 |
| [多 Agent 并行执行计划](06-多Agent并行执行计划.md) | 当前 | 当前唯一依赖、文件所有权、任务规格、批次和停止条件 |
| [开发任务状态](../../tasks/todo.md) | 当前 | 面向多 Agent 领取的逐项状态看板 |
| [开发日志](02-开发日志.md) | 持续更新 | 每个完成切片的代码、接口、DDL、事件、测试和风险证据 |
| [2026-07-16 今日开发计划](04-今日开发计划-2026-07-16.md) | 历史 | 记录前端、计划治理、旧统一报表和外部阻塞项的承接过程 |
| [2026-07-13 缺口收口计划](03-今日开发计划-2026-07-13.md) | 历史 | 记录安全、可靠性、调拨、逆向、前端、生产联调等缺口来源 |

### 3.2 九个业务系统接口级计划

九个系统的接口级规格均已完成首轮仓库实现。这里的“首轮完成”不等于生产上线，生产完成仍需跨领域需求和 Definition of Done 验收。

| 范围 | 编号 | 状态 | 详细入口 |
| --- | --- | --- | --- |
| 供应商、采购、WMS | SUP/PUR/WMS | 首轮完成 | [供应商](01-开发计划/01-供应商系统接口级开发计划.md)、[采购](01-开发计划/02-采购系统接口级开发计划.md)、[WMS](01-开发计划/03-WMS系统接口级开发计划.md) |
| 中央库存、OMS、TMS | INV/OMS/TMS | 首轮完成 | [中央库存](01-开发计划/04-中央库存系统接口级开发计划.md)、[OMS](01-开发计划/05-OMS系统接口级开发计划.md)、[TMS](01-开发计划/06-TMS系统接口级开发计划.md) |
| BMS、主数据、权限 | BMS/MDM/IAM | 首轮完成 | [BMS](01-开发计划/07-BMS系统接口级开发计划.md)、[主数据](01-开发计划/08-主数据系统接口级开发计划.md)、[权限](01-开发计划/09-权限系统接口级开发计划.md) |

### 3.3 跨系统需求分类索引

| 类别 | 需求编号 | 当前状态 | 核心依赖 | 验收入口 |
| --- | --- | --- | --- | --- |
| 集成与可靠性 | INT-REQ-001 至 005A | 已收敛 | 能力按数据主权回归九个子系统；各系统保留 Outbox/Inbox、重试、熔断和人工重放 | [ARCH-REQ-001](需求单/ARCH-REQ-001-九子系统收敛与Java规范治理.md)、`ARCH-LOG-001` |
| 真实外部联调 | INT-REQ-005B | 阻塞 | RocketMQ、Nacos/Dubbo、ERP/税务/支付沙箱 | [INT-REQ-005B](需求单/INT-REQ-005B-RocketMQ-Dubbo与财税支付真实联调.md) |
| 环境与配置中心 | CFG-REQ-001 | 已完成 | Spring Profiles、Nacos Config、MySQL/Redis/RocketMQ/Dubbo | [CFG-REQ-001](需求单/CFG-REQ-001-TestProd环境与Nacos优先配置.md)、`CFG-LOG-001` |
| 前端应用 | FE-REQ-001 至 010 | 已完成 | 九大子系统工作台、独立列表/详情路由、各业务 API、IAM 登录/动态菜单/功能权限及四档响应式验收 | [FE-REQ-001](需求单/FE-REQ-001-供应链运营工作台与系统导航.md)、[FE-REQ-009](需求单/FE-REQ-009-完整业务菜单登录态与IAM动态权限联调.md)、[FE-REQ-010](需求单/FE-REQ-010-九大子系统标准工作台列表详情前端重构.md)、`FE-LOG-001~010` |
| QA | QA-REQ-001、002 | 部分完成 | 仓库内 Playwright、MySQL/Flyway 基线已完成；真实 API/外部系统联调仍待 `QA-NEXT-001`、`INT-REQ-005B` | [QA-REQ-001](需求单/QA-REQ-001-真实浏览器权限E2E与生产化集成测试.md)、[QA-REQ-002](需求单/QA-REQ-002-Spring-Web与MySQL集成测试基线.md)、`QA-LOG-001/002` |
| 经营分析与报表 | RPT-REQ-001 | 已拆分 | OMS、库存、BMS、供应商分别拥有履约、库存、成本、供应商绩效查询与导出 | [ARCH-REQ-001](需求单/ARCH-REQ-001-九子系统收敛与Java规范治理.md) |
| 架构与代码规范 | ARCH-REQ-001 | 已完成 | 九子系统边界、JDK 17、阿里 Java 编码规范、全量注释 | [ARCH-REQ-001](需求单/ARCH-REQ-001-九子系统收敛与Java规范治理.md)、`ARCH-LOG-001` |
| 生产运维 | OPS-REQ-001 | 阻塞 | 可运行集成/预演环境、监控和告警通道 | [OPS-REQ-001](需求单/OPS-REQ-001-生产监控备份恢复与上线演练.md) |
| 计划治理 | DOC-REQ-001 | 已完成 | 需求单、今日计划和日志一致 | [DOC-REQ-001](需求单/DOC-REQ-001-总计划分类索引与追踪治理.md) |

### 3.4 状态口径

- `已完成`：需求单验收项在仓库内有实现和测试证据。
- `待开发`：依赖具备但尚未开始。
- `进行中`：已被 Agent 领取的活动切片；允许多个无依赖且文件所有权不重叠的切片并行。
- `阻塞`：缺少仓库外环境、地址、凭证或测试数据，禁止以本地模拟冒充完成。
...
- 当前状态事实：[`05-九子系统模块状态矩阵.md`](05-九子系统模块状态矩阵.md)。
- 当前并行执行计划：[`06-多Agent并行执行计划.md`](06-多Agent并行执行计划.md)。
- 逐项领取看板：[`../../tasks/todo.md`](../../tasks/todo.md)。
- 2026-07-13、2026-07-16 计划只作历史快照，不再更新为当前状态。
- 总计划中的“首轮完成”仅表示仓库内代码切片覆盖；是否生产完成，以当前多 Agent 计划的任务状态、检查点和 Definition of Done 为准。

## 继续上下文

当前结论：九个业务系统已完成首轮仓库实现，二期缺口已经按子系统和模块拆为可并行领取的 `NEXT` 任务。
关键假设：外部中间件、财税支付沙箱和上线预演环境尚未提供。
待决问题：`INT-REQ-005B`、`OPS-REQ-001` 的地址、凭证、Topic、告警通道和测试数据。
下一步：先并行完成 `PAR-NEXT-001/002`，通过 CP-0 后进入供应商、采购、WMS、库存批次。
