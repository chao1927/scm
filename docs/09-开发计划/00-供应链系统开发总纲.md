...
4. 基础设施：Repository/Mapper、Flyway、Outbox/Inbox、缓存或外部端口。
...
| 数据库 | 九个服务独立 schema；Flyway `V{序号}__{说明}.sql` |
...
| 权限   | Redis 会话、密钥轮换、OAuth/OIDC、MFA 登录闭环、治理读模型与真实 RocketMQ 投递链已完成 | 仓库阶段完成 | 最终 QA | 真实 Redis/MySQL/Flyway/RocketMQ 和轮换演练 |
...
| Flyway 目录 | 同一服务同一时间一个写入者，领取时登记版本号 |
...
| 项目 | 内容 |
| --- | --- |
| 目标 | 验证准入、PO、ASN、质量和导出的 Web/MySQL/Flyway 契约 |
| 依赖 | `SUP-NEXT-001A` |
...
| 验证 | IAM/scm-common/九资源服务测试、伪造 `kid`/旧密钥/会话串用威胁测试、Ali Check、真实 Redis/Flyway 冒烟 |
...
| 项目 | 内容 |
| --- | --- |
| 目标 | 使用本地九服务、MySQL/Flyway、Nacos 和浏览器形成真实基线 |
...
4. 九服务和 MySQL/Flyway 启动，核心查询与写命令返回真实数据库结果。
...
5. 九服务和 MySQL/Flyway 启动，核心查询与写命令返回真实数据库结果。
...
| `WMS-NEXT-002` | `V12__wms_multitype_inbound.sql`、三类来源 ACL/复合唯一键、调拨差异、售后五类处置与 RocketMQ Outbox/Inbox | scm-common 9/9、WMS 50/50，JDK 17 通过 | 真实 MySQL/Flyway/RocketMQ 归入 `QA-NEXT-001`；存量 Ali Check 63 项待治理 |
| `MDM RocketMQ` | `V7__mdm_rocketmq_delivery.sql`、动态发布 Topic、真实 Producer/Consumer、Outbox 退避和 Inbox 幂等 | scm-common 9/9、MDM 51/51，JDK 17 构建通过 | 真实 RocketMQ/MySQL/Flyway 归入 `QA-NEXT-001`；MDM 存量 Ali Check 还有 59 项待全量注释治理 |
| `IAM-NEXT-001A/C` | MFA 绑定登录和幂等会话签发；菜单/角色/会话/MFA 治理读模型；`V9__iam_rocketmq_delivery.sql` 及真实 RocketMQ Producer/Consumer | scm-common 9/9、IAM 47/47、Ali Check；前端 43/43 和 4953 模块构建通过 | 真实 Redis/MySQL/Flyway/RocketMQ 和密钥轮换演练归入 `QA-NEXT-001` |
...
| `BMS-NEXT-002` | `V6__bms_refund_settlement_hardening.sql`；请求幂等/CAS、待确认占额、回执事实校验与异常队列、失败重试重校验、人工双人处置及 Outbox/审计 | scm-common 9/9、BMS 24/24；JDK 17 编译与 `git diff --check` 通过 | 真实 MySQL 并发行锁、Flyway 和支付查单需在 `QA-NEXT-001` 实跑；BMS 存量 Ali Check 问题待全量门禁清理 |
| `PUR-NEXT-002` | 价格适用范围按 `priceType` 隔离；截标后报价失败关闭；存在 ASN 禁止取消；PO/变更单 CAS 与 `base_order_version` V13 迁移 | Purchase 81/81、含 scm-common 90/90；专项 12/12；`git diff --check` 通过 | 真实 MySQL Flyway 与跨服务 ASN 事实需在最终 QA 实跑 |
| `INV-NEXT-002` | 调拨差异原因/责任方/证据持久化；收货与上架事实分离；乱序事件进入待重放状态且不重复记账 | Inventory 43 个测试通过，1 个真实 MySQL 条件测试因未启用测试容器跳过；计划校验通过 | 需在真实 MySQL/Flyway 与跨进程 WMS 事件链路做最终冒烟 |
