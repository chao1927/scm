...
## INV-API-006 调拨申请、审批、预占、在途与差异闭环
`POST/GET /transfers`、`GET /transfers/{transferNo}`、`POST /transfers/{transferNo}/submit|approve|reserve|cancel`；调拨事实由 RocketMQ `inventory-domain-event-consumer` 消费组接收

- 接口层：`StockTransferController` 校验货主、源仓、目标仓、SKU、数量、版本和创建幂等键；管理接口按源仓和目标仓双向校验数据范围。
- 应用层：`StockTransferApplicationService` 编排提交、审批、库存预占、WMS 出库命令、TMS 在途事实、WMS 调入上架事实和差异确认。
- 领域层：`StockTransferAggregate` 保护 `申请量 >= 预占量 >= 出库量`、`出库量 = 在途待处理量 + 已入库量 + 已确认差异量`；调入仓上架前不得增加可用库存。
- 基础设施层：调拨头行资源库、创建幂等表、乐观锁、库存预占/流水 ACL、Inbox/Outbox、调拨读模型。
- 事件：生产 `TransferCreated/Submitted/Approved/StockReserved/OutboundCompleted/InTransit/InboundCompleted/DifferenceConfirmed/Cancelled`；消费 WMS/TMS 调拨执行事实。
- 补偿：预占后取消释放库存；出库后不得直接取消，必须按拒收、退回、差异或异常关闭处理；重复和乱序执行事实不得重复改变数量。
- 测试：聚合数量守恒、状态迁移、并发审批/取消、重复事件、少收/多收/破损和双仓数据权限。
- 设计事实源：[调拨单聚合 CQRS 设计](../../03-核心业务模型/04-中央库存领域模型/08-调拨单聚合CQRS设计.md)。实现验收还必须证明 TMS 事实不记账、乱序事件待重放、上架前目标仓可用量不增加。
