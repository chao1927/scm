...
`GET /openapi/master-data/{type}`、校验/快照接口；内部业务事件由 RocketMQ `mdm-business-event-consumer` 消费组接收
...
## MDM RocketMQ 投递链交付证据（2026-08-06）

- `V7__mdm_rocketmq_delivery.sql` 为 Outbox 增加重试、下次投递时间、错误和 Broker 确认时间。
- `MdmOutboxDispatchApplicationService` 只在 RocketMQ 确认后标记成功；发布订阅事件使用订阅指定 Topic，其他领域事件使用 MDM 默认 Topic。
- 生产环境仅注册真实 `RocketMqMdmMessageBrokerAdapter` 和 `RocketMqMdmInboundEventConsumer`；Topic 未配置、未知 schema 版本或业务失败均进入 Broker/Outbox 重试，不提供 Noop/日志降级。
- JDK 17 下 `scm-common 9/9 + MDM 51/51` 通过；真实 RocketMQ/MySQL 完整 schema 运行验证归入 `QA-NEXT-001`。
