...
**需求基线**：[SUP-REQ-001 供应商核心业务需求基线](../需求单/SUP-REQ-001-供应商核心业务需求基线.md)。

**实现资料**：`docs/08-系统实现/01-供应商系统实现/03-供应商系统接口逐项实现设计.md`。所有接口复用 [接口级执行公共契约](00-接口级执行公共契约.md)，本文件记录供应商上下文的特有逻辑。

`SUP-API-001~008` 必须全部回链到 `SUP-REQ-001` 的准入资质、供货能力、协同履约、质量风险和财务协同验收；不能以接口存在替代正式业务需求验收。
...
## SUP-NEXT-002 需求双向追踪与异常回归证据

> 验收日期：2026-08-06。`SUP-REQ-001` 是供应商核心业务不变量的正式事实来源。

| 接口切片 | 需求/不变量 | 关键实现 | 自动化证据 |
| --- | --- | --- | --- |
| `SUP-API-001` | 工作台指标只读事实投影，且按供应商和组织范围隔离 | `SupplierWorkbenchApplicationService`、`SupplierWorkbenchMapper` | `SupplierWorkbenchApplicationServiceTest`、`SupplierSecurityAndScopeWebTest` |
| `SUP-API-002` | 未通过准入不得启用；关键档案变更必须留存版本和审批事实 | `SupplierAdmissionAggregate`、`ProfileChangeAggregate`、`ProfileApplicationService` | `SupplierAdmissionAggregateTest`、`ProfileChangeAggregateTest` |
| `SUP-API-003` | 账号绑定保护主账号和供应商数据范围；生命周期变更交由主数据主权执行 | `SupplierUserBindingApplicationService`、`SupplierLifecycleApplicationService`、集成命令 Outbox | `SupplierLifecycleApplicationServiceTest.shouldEnqueueSupplierFreezeCommand`、`SupplierSecurityAndScopeWebTest` |
| `SUP-API-004` | 资质失效、冻结或停供时不得建立新采购引用；MOQ/MPQ/交期必须有效 | `SupplierQualificationAggregate`、`SupplierItemAggregate`、`SupplyCondition` | `SupplierQualificationAggregateTest`、`SupplierItemAggregateTest` |
| `SUP-API-005` | 报价受 RFQ 截标约束；合同审批、生效、续签、终止和到期受状态机保护 | `SupplierQuoteAggregate`、`SupplierContractAggregate`、价格协议投影 | `SupplierQuoteAggregateTest`、`SupplierContractAggregateTest` |
| `SUP-API-006` | PO 确认数量/交期/差异可追踪；ASN 重复提交不得再次触发下游 | `PoConfirmAggregate`、`AsnAggregate`、`AsnCommandApplicationService` | `PoConfirmAggregateTest.shouldReportPurchaseOrderLineDifference`、`AsnAggregateTest.shouldRejectDuplicateAsnSubmission` |
| `SUP-API-007` | 整改逾期进入风险状态；退供不超可退数量；对账差异缺原因禁止写入 | `SupplierQualityIssueAggregate`、`SupplierReturnAggregate`、`SupplierFinanceApplicationService`、评分域服务 | `SupplierQualityIssueAggregateTest.shouldMarkRectificationOverdue`、`SupplierReturnAggregateTest`、`SupplierFinanceApplicationServiceTest`、`SupplierScoringDomainServiceTest` |
| `SUP-API-008` | 运营查询不绕过聚合写状态；导出、重放、Inbox/Outbox 可追踪且受权限保护 | `SupplierOperationsApplicationService`、`OutboxDispatchApplicationService`、RocketMQ Producer/Consumer | `SupplierOperationsExportTest`、`SupplierExportProcessorTest`、`OutboxDispatchApplicationServiceTest`、`RocketMqProductionBeanContractTest` |

### 数据范围与可靠协同结论

- 写命令通过 `CommandContext.requireSupplierScope` 约束供应商范围，Web 层同时验证 401/403 和跨供应商拒绝。
- 冻结、IAM 绑定、WMS/TMS/BMS 协同均保持数据主权边界；业务事件使用真实 RocketMQ Producer/Consumer，不使用日志或 Noop 伪投递。
- Outbox/Inbox、幂等键、请求摘要和乐观锁共同防止重复命令、重复事件与并发覆盖。

### 验证命令

```bash
cd project/backend
JAVA_HOME='/Applications/IntelliJ IDEA.app/Contents/jbr/Contents/Home' mvn -o -pl supplier-service -am -DforkCount=0 test
```

结果：scm-common `9/9`；Supplier 共收集 `64` 项，已执行 `61/61` 通过，`3` 项真实 MySQL 条件测试因当前未配置数据库而跳过，归入 `QA-NEXT-001` 实环境门禁。
